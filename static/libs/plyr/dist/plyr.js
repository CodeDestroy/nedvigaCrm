typeof navigator === "object" && (function (global, factory) {
    ...
    let RangeTouch = function () {
        ...
        return _createClass(e, [{
            ...
            value: function (t) {
                ...
                let n, r = t.target, i = t.changedTouches[0],
    ...
    function setAttributes(element, attributes) {
        ...
        // Assume null and undefined attributes should be left out, Setting them would otherwise convert them to "null" and "undefined"
    ...
    function toggleListener(element, event, callback, toggle = false, passive = true, capture = false) {
        ...
        events.forEach(type => {
            if (this && this.eventListeners && toggle) this.eventListeners.push({element, type, callback, options}); // Cache event listener
    ...
    const controls = {
        ...
        bindMenuItemShortcuts(menuItem, type) {
            ...
            // Enter will fire a `click` event but we still need to manage focus So we bind to keyup which fires after and set focus here
        ...
        inject() {
            ...
            if (is.element(this.config.controls) || is.string(this.config.controls)) container = this.config.controls; // HTMLElement or Non-empty string passed as the option
            else {
                // Create controls
    ...
    const ui = {
        ...
        setPoster(poster, passive = true) {
            ...
            return ready.call(this).then(() => loadImage(poster)).catch(error => { // Load image
        ...
        (function (root, factory) {{
            ...
            function loadFile(path, callbackFn, args, numTries) {
                ...
                e.onload = e.onerror = e.onbeforeload = function (ev) {
                    ...
                    } else if (e.rel === 'preload' && e.as === 'style') return e.rel = 'stylesheet'; // jshint ignore:line
                    // execute callback
     ...
     */
        const regex = /^.*(vimeo.com\/|video\/)(\d+)(\?.*&*h=|\/)+([\d,a-f]+)/, found = url.match(regex);
        ...
        ready() {
            ...
            Object.defineProperty(player.media, 'currentTime', {
                ...
                set(time) {
                    ...
                    const {embed, media, paused, volume} = player, restorePause = paused && !embed.hasPlayed;
            ...
            player.embed.on('cuechange', ({cues = []}) => {
                captions.updateCues.call(player, cues.map(cue => stripHTML(cue.text)));
            });
    ...
    class Ads {
        ...
        constructor(player) {
            ...
            _defineProperty$1(this, "onAdEvent", event => {
                ...
                switch (event.type) {
                    ...
                    case google.ima.AdEvent.Type.ALL_ADS_COMPLETED:
                        // All ads for the current videos are done. We can now request new advertisements in case the video is re-played
                        // TODO: Example for what happens when a next video in a playlist would be loaded.
                        // So here we load a new video when all ads are done. Then we load new ads within a new adsManager.
                        // When the video Is started - after - the ads are loaded, then we get ads.
            ...
            /**
             * Setup a safety timer for when the ad network doesn't respond for whatever reason. The advertisement has
             * 12 seconds to get its things together. We stop this timer when the advertisement is playing, or when a
             * user action is required to start, then we clear the timer on ad ready
             * @param {Number} time
             * @param {String} from
             */
        ...
        constructor(player) {
            ...
            _defineProperty$1(this, "startMove", event => {
                ...
                if (event.type === 'touchmove') this.seekTime = this.player.media.duration * (this.player.elements.inputs.seek.value / 100);
                    ...
                    // Append the point label to the tooltip
                    if (point) this.elements.thumb.time.insertAdjacentHTML('afterbegin', `${point.label}<br>`); // this.elements.thumb.time.innerText.concat('\n');
                }
            ...
            _defineProperty$1(this, "loadImage", (qualityIndex = 0) => {
                const thumbNum = this.showingThumb, thumbnail = this.thumbnails[qualityIndex];
                const {urlPrefix} = thumbnail, frame = thumbnail.frames[thumbNum];
                const thumbFilename = thumbnail.frames[thumbNum].text, thumbUrl = urlPrefix + thumbFilename;
    ...
    const source = {
        ...
        change(input) {
            ...
            this.destroy.call(this, () => {
                ...
                if (this.isHTML5 || this.isEmbed && !this.supported.ui) ui.build.call(this); // Setup interface
                // Load HTML5 sources
