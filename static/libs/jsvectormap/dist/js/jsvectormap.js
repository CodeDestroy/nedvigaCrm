(function (global, factory) {
    ...
    // Object.assign polyfill https://gist.github.com/spiralx/68cf40d7010d829340cb
    if (!Object.assign) {
    ...
    let deepmerge = function deepmerge(target, source, options) {
        options = options || {};
        ...
        let sourceIsArray = Array.isArray(source), targetIsArray = Array.isArray(target);
     ...
     * --------------------------------------------------------------------------
     */
    let getElement = function getElement(selector) {
        if (typeof selector === 'object' && typeof selector.nodeType !== 'undefined') return selector;
        if (typeof selector === 'string') return document.querySelector(selector);
        return null;
    };
    let createElement = function createElement(type, classes, content, html) {
    ...
    let findElement = function findElement(parentElement, selector) {
        return Element.prototype.querySelector.call(parentElement, selector);
    };
    let removeElement = function removeElement(target) {
        target.parentNode.removeChild(target);
    };
    let isImageUrl = function isImageUrl(url) {
        return /\.(jpg|gif|png)$/.test(url);
    };
    let hyphenate = function hyphenate(string) {
        return string.replace(/[\w]([A-Z])/g, (m) => { return m[0] + "-" + m[1]; }).toLowerCase();
    };
    let merge = function merge(target, source, deep) {
        if (deep === void 0) deep = false;
        if (deep) return deepmerge(target, source);
        return Object.assign(target, source);
    };
    let getLineUid = function getLineUid(from, to) {
        return from.toLowerCase() + ":to:" + to.toLowerCase();
    };
    let inherit = function inherit(target, source) {
        Object.assign(target.prototype, source);
    };
    let eventRegistry = {}, eventUid = 1;
    /**
     * ------------------------------------------------------------------------
     * Event Handler
     * ------------------------------------------------------------------------
    ...
    let EventHandler = {
        on: function on(element, event, handler, options) {
            ...
            let uid = "jvm:" + event + "::" + eventUid++;
            eventRegistry[uid] = {selector: element, handler: handler};
            handler._uid = uid;
    ...
    function setupContainerEvents() {
        let _this = this, map = this, mouseDown = false;
        ...
        if (this.params.zoomOnScroll) {
            EventHandler.on(this.container, 'wheel', function (event) {
                let deltaY = ((event.deltaY || -event.wheelDelta || event.detail) >> 10 || 1) * 75;
                let rect = _this.container.getBoundingClientRect();
                let offsetX = event.pageX - rect.left - window.pageXOffset;
                let offsetY = event.pageY - rect.top - window.pageYOffset;
                let zoomStep = Math.pow(1 + map.params.zoomOnScrollSpeed / 1000, -1.5 * deltaY);
    ...
    let parseEvent = function parseEvent(map, selector, isTooltip) {
        let element = getElement(selector);
        let type = element.getAttribute('class').indexOf('jvm-region') === -1 ? 'marker' : 'region';
        let isRegion = type === 'region';
        let code = isRegion ? element.getAttribute('data-code') : element.getAttribute('data-index');
    ...
    function setupElementEvents() {
        let map = this, container = this.container, pageX, pageY, mouseMoved;
        ...
        EventHandler.delegate(container, 'mouseover mouseout', '.jvm-element', function (event) {
            let data = parseEvent(map, this, true), showTooltip = map.params.showTooltip;
        ...
        EventHandler.delegate(container, 'mouseup', '.jvm-element', function (event) {
            ...
            if (data.type === 'region' && map.params.regionsSelectable || data.type === 'marker' && map.params.markersSelectable) {
                ...
                if (map.params[data.type + "sSelectableOne"]) map._clearSelected(data.type + "s");
        ...
        EventHandler.delegate(container, 'click', '.jvm-element', function (event) {
            let _parseEvent = parseEvent(map, this), type = _parseEvent.type, code = _parseEvent.code;
            map._emit(type === 'region' ? Events.onRegionClick : Events.onMarkerClick, [event, code]);
        });
    }
    function setupZoomButtons() {
        let _this = this;
        let zoomin = createElement('div', 'jvm-zoom-btn jvm-zoomin', '&#43;', true);
        let zoomout = createElement('div', 'jvm-zoom-btn jvm-zoomout', '&#x2212', true);
        this.container.appendChild(zoomin);
        this.container.appendChild(zoomout);
        let handler = function handler(zoomin) {
    ...
    function setupContainerTouchEvents() {
        let map = this, touchStartScale, touchStartDistance, touchX, touchY, centerTouchX,
            centerTouchY, lastTouchesLength;
        let handleTouchEvent = function handleTouchEvent(e) {
            let touches = e.touches, offset, scale, transXOld, transYOld;
    ...
    let BaseComponent = /*#__PURE__*/function () {
        ...
        _proto.dispose = function dispose() {
            ...
            for (let _iterator = _createForOfIteratorHelperLoose(Object.getOwnPropertyNames(this)), _step; !(_step = _iterator()).done;) {
                let propertyName = _step.value;
    ...
    let Interactable = {
        getLabelText: function getLabelText(key, label) {
            ...
            if (typeof label.render === 'function') {
                ...
                if (this.config && this.config.marker) params.push(this.config.marker);
     ...
     * ------------------------------------------------------------------------
     */
    let Region = /*#__PURE__*/function (_BaseComponent) {
        _inheritsLoose(Region, _BaseComponent);
        function Region(_ref) {
            let _this, map = _ref.map, code = _ref.code, path = _ref.path, style = _ref.style, label = _ref.label,
                labelStyle = _ref.labelStyle, labelsGroup = _ref.labelsGroup;
            _this = _BaseComponent.call(this) || this;
            _this._map = map;
            _this.shape = _this._createRegion(path, code, style);
            let text = _this.getLabelText(code, label);
            // If label is passed and render function returns something
            if (label && text) {
                let bbox = _this.shape.getBBox(), offsets = _this.getLabelOffsets(code, label);
        ...
        _proto.updateLabelPosition = function updateLabelPosition() {
            if (this.label) this.label.set({x: this.labelX * this._map.scale + this._map.transX * this._map.scale, y: this.labelY * this._map.scale + this._map.transY * this._map.scale});
        };
        return Region;
    }(BaseComponent);
    ...
    function createRegions() {
        ...
        for (let code in this._mapData.paths) {
            ...
            this.regions[code] = {config: this._mapData.paths[code], element: region};
        }
    }
    /**
     * ------------------------------------------------------------------------
     ...
     * ------------------------------------------------------------------------
     */
    let Line = /*#__PURE__*/function (_BaseComponent) {
        _inheritsLoose(Line, _BaseComponent);
        function Line(_ref) {
            let _this, index = _ref.index, map = _ref.map, style = _ref.style, x1 = _ref.x1, y1 = _ref.y1,
                x2 = _ref.x2, y2 = _ref.y2, group = _ref.group, config = _ref.config;
            _this = _BaseComponent.call(this) || this;
            _this.config = config;
            _this.shape = map.canvas.createLine({x1: x1, y1: y1, x2: x2, y2: y2, dataIndex: index}, style, group);
            _this.shape.addClass('jvm-line');
            return _this;
        }
        let _proto = Line.prototype;
    ...
    function createLines(lines, markers, isRecentlyCreated) {
        ...
        // Create group for holding lines we're checking if `linesGroup` exists or not becuase we may add lines
        ...
        for (let index in lines) {
            let config = lines[index];
            for (let mindex in markers) {
                let markerConfig = isRecentlyCreated ? markers[mindex].config : markers[mindex];
                if (markerConfig.name === config.from) point1 = this.getMarkerPosition(markerConfig);
                if (markerConfig.name === config.to) point2 = this.getMarkerPosition(markerConfig);
            }
     ...
     * ------------------------------------------------------------------------
     */
    let Marker = /*#__PURE__*/function (_BaseComponent) {
        _inheritsLoose(Marker, _BaseComponent);
        function Marker(_ref) {
            let _this, index = _ref.index, style = _ref.style, label = _ref.label, cx = _ref.cx, cy = _ref.cy,
                map = _ref.map, group = _ref.group;
            _this = _BaseComponent.call(this) || this;
            // Private
            _this._map = map;
            ...
            _this.shape = map.canvas[_this._isImage ? 'createImage' : 'createCircle']({dataIndex: index, cx: cx, cy: cy}, style, group);
        ...
        _proto._createLabel = function _createLabel(_ref2) {
            let index = _ref2.index, map = _ref2.map, label = _ref2.label, labelsGroup = _ref2.labelsGroup, cx = _ref2.cx,
                cy = _ref2.cy, marker = _ref2.marker, isRecentlyCreated = _ref2.isRecentlyCreated;
            ...
            this.label = map.canvas.createText({text: labelText, dataIndex: index, x: this._labelX, y: this._labelY, dy: '0.6ex'}, map.params.markerLabelStyle, labelsGroup);
    ...
    function createMarkers(markers, isRecentlyCreated) {
        ...
        if (isRecentlyCreated === void 0) isRecentlyCreated = false;
        // Create groups for holding markers and markers labels. We're checking if `markersGroup` exists or not, becuase
        // we may add markers after the map has loaded. So we will append the futured markers to this group as well.
        ...
        let _loop = function _loop() {
            let config = markers[index], point = _this.getMarkerPosition(config), uid = config.coords.join(':');
            if (!point) return "continue";
            ...
            if (isRecentlyCreated) {
                if (Object.keys(_this._markers).filter((i) => { return _this._markers[i]._uid === uid; }).length) return "continue";
            ...
            let marker = new Marker({
                ...
                style: merge(_this.params.markerStyle, {initial: config.style || {}}, true),
            ...
            // Check for marker duplication this is useful when for example: a user clicks a button for creating marker
            // two times so it will remove the old one and the new one will take its place.
            if (_this._markers[index]) _this.removeMarkers([index]);
            _this._markers[index] = {_uid: uid, config: config, element: marker};
        };
        for (let index in markers) {
            if (_loop() === "continue") continue;
        }
    }
    /**
     * ------------------------------------------------------------------------
    ...
    let Legend = /*#__PURE__*/function () {
        ...
        let _proto = Legend.prototype;
        _proto.render = function render() {
            let ticks = this._series.scale.getTicks(), inner = createElement('div', 'jvm-legend-inner');
            this._body.innderHTML = '';
            if (this._options.title) this._body.appendChild(createElement('div', 'jvm-legend-title', this._options.title));
            ...
            for (let i = 0; i < ticks.length; i++) {
                ...
                let tickText = createElement('div', 'jvm-legend-tick-text', label);
     ...
     * ------------------------------------------------------------------------
     */
    let OrdinalScale = /*#__PURE__*/function () {
        function OrdinalScale(scale) {
            this._scale = scale;
        }
        let _proto = OrdinalScale.prototype;
    ...
    let Series = /*#__PURE__*/function () {
        ...
        let _proto = Series.prototype;
        _proto.setValues = function setValues(values) {
            let attrs = {};
            for (let key in values) {
                if (values[key]) attrs[key] = this.scale.getValue(values[key]);
            }
            this.setAttributes(attrs);
        };
        _proto.setAttributes = function setAttributes(attrs) {
            for (let code in attrs) {
                if (this._elements[code]) this._elements[code].element.setStyle(this.config.attribute, attrs[code]);
            }
        };
        _proto.clear = function clear() {
            let key, attrs = {};
            for (key in this._values) {
                if (this._elements[key]) attrs[key] = this._elements[key].element.shape.style.initial[this.config.attribute];
            }
            this.setAttributes(attrs);
            this._values = {};
        };
    ...
    function resize() {
        let curBaseScale = this._baseScale;
    ...
    function setScale(scale, anchorX, anchorY, isCentered, animate) {
        let _this = this, zoomStep, interval, i = 0,
        ...
        if (codes.length) {
            codes.forEach(function (code) {
                if (_this.regions[code]) {
                    let itemBbox = _this.regions[code].element.shape.getBBox();
                    if (itemBbox) {
                        // Handle the first loop
                        if (typeof bbox == 'undefined') bbox = itemBbox;
                        else {
                            // get the old bbox properties plus the current this kinda incrementing the old values and the new values
                            bbox = {
            ...
            let point = this.coordsToPoint(config.coords[0], config.coords[1]);
            let x = this.transX - point.x / this.scale, y = this.transY - point.y / this.scale;
    ...
    let Proj = {
        ...
        aea: function aea(lat, lng, c) {
            let fi0 = 0, lambda0 = c * this.radDeg, fi1 = 29.5 * this.radDeg,
                fi2 = 45.5 * this.radDeg, fi = lat * this.radDeg, lambda = lng * this.radDeg,
                n = (Math.sin(fi1) + Math.sin(fi2)) / 2, C = Math.cos(fi1) * Math.cos(fi1) + 2 * n * Math.sin(fi1),
                theta = n * (lambda - lambda0), ro = Math.sqrt(C - 2 * n * Math.sin(fi)) / n,
                ro0 = Math.sqrt(C - 2 * n * Math.sin(fi0)) / n;
            return {x: ro * Math.sin(theta) * this.radius, y: -(ro0 - ro * Math.cos(theta)) * this.radius};
        },
        /* aea_inv(xCoord, yCoord, c) {
          let x = xCoord / this.radius, y = yCoord / this.radius, fi0 = 0, lambda0 = c * this.radDeg,
        ...
        lcc: function lcc(lat, lng, c) {
            let fi0 = 0, lambda0 = c * this.radDeg, lambda = lng * this.radDeg,
    ...
    function coordsToPoint(lat, lng) {
        let projection = Map.maps[this.params.map].projection;
        let _Proj$projection$type = Proj[projection.type](lat, lng, projection.centralMeridian),
            x = _Proj$projection$type.x, y = _Proj$projection$type.y, inset = this.getInsetForPoint(x, y);
        if (!inset) return false;
        let bbox = inset.bbox;
        x = (x - bbox[0].x) / (bbox[1].x - bbox[0].x) * inset.width * this.scale;
        y = (y - bbox[0].y) / (bbox[1].y - bbox[0].y) * inset.height * this.scale;
        return {x: x + this.transX * this.scale + inset.left * this.scale, y: y + this.transY * this.scale + inset.top * this.scale};
    }
    function getInsetForPoint(x, y) {
        let insets = Map.maps[this.params.map].insets;
        for (let index = 0; index < insets.length; index++) {
            let _insets$index$bbox = insets[index].bbox, start = _insets$index$bbox[0], end = _insets$index$bbox[1];
    ...
    function repositionLines() {
        ...
        for (let index in this._lines) {
            for (let mindex in this._markers) {
                let marker = this._markers[mindex];
                if (marker.config.name === this._lines[index].config.from) point1 = this.getMarkerPosition(marker.config);
                if (marker.config.name === this._lines[index].config.to) point2 = this.getMarkerPosition(marker.config);
            }
            if (point1 !== false && point2 !== false) this._lines[index].setStyle({x1: point1.x, y1: point1.y, x2: point2.x, y2: point2.y});
        }
    }
    function repositionMarkers() {
        for (let index in this._markers) {
            let point = this.getMarkerPosition(this._markers[index].config);
            if (point !== false) this._markers[index].element.setStyle({cx: point.x, cy: point.y});
        }
    }
    function repositionLabels() {
        let labels = this.params.labels;
        if (!labels) return;
        // Regions labels
        if (labels.regions) {
    ...
    let Defaults = {
        ...
        markerStyle: {
            initial: {r: 7, fill: '#374151', fillOpacity: 1, stroke: '#FFF', strokeWidth: 5, strokeOpacity: .5},
     ...
     * ------------------------------------------------------------------------
     */
    let SVGElement = /*#__PURE__*/function () {
        ...
        let _proto = SVGElement.prototype;
     ...
     * ------------------------------------------------------------------------
     */
    let SVGShapeElement = /*#__PURE__*/function (_SVGElement) {
        ...
        let _proto = SVGShapeElement.prototype;
        _proto.setStyle = function setStyle(property, value) {
     ...
     * ------------------------------------------------------------------------
     */
    let SVGTextElement = /*#__PURE__*/function (_SVGShapeElement) {
        ...
        let _proto = SVGTextElement.prototype;
     ...
     * ------------------------------------------------------------------------
     */
    let SVGImageElement = /*#__PURE__*/function (_SVGShapeElement) {
        ...
        let _proto = SVGImageElement.prototype;
        _proto.applyAttr = function applyAttr(attr, value) {
            let imageUrl;
            if (attr === 'image') {
            ...
            } else _SVGShapeElement.prototype.applyAttr.apply(this, arguments); // This time Call SVGElement
        };
     ...
     * ------------------------------------------------------------------------
     */
    let SVGCanvasElement = /*#__PURE__*/function (_SVGElement) {
        ...
        let _proto = SVGCanvasElement.prototype;
        ...
        _proto.createPath = function createPath(config, style) {
            let path = new SVGShapeElement('path', config, style);
        ...
        _proto.createCircle = function createCircle(config, style, group) {
            return this._add(new SVGShapeElement('circle', config, style), group);
        };
        // Create `line` element
        _proto.createLine = function createLine(config, style, group) {
            return this._add(new SVGShapeElement('line', config, style), group);
        };
        // Create `text` element
        _proto.createText = function createText(config, style, group) {
            return this._add(new SVGTextElement(config, style), group);
        };
        // Create `image` element
        _proto.createImage = function createImage(config, style, group) {
            return this._add(new SVGImageElement(config, style), group);
        };
        // Create `g` element
    ...
    let Tooltip = /*#__PURE__*/function (_BaseComponent) {
        ...
        function Tooltip(map) {
            let _this;
            _this = _BaseComponent.call(this) || this;
            let tooltip = createElement('div', 'jvm-tooltip');
            _this._map = map;
        ...
        _proto._bindEventListeners = function _bindEventListeners() {
            ...
            EventHandler.on(this._map.container, 'mousemove', function (event) {
                ...
                let container = findElement(_this2._map.container, '#jvm-regions-group').getBoundingClientRect();
                let space = 5; // Space between the cursor and tooltip element
                // Tooltip
                let _this2$_tooltip$getBo = _this2._tooltip.getBoundingClientRect(),
                    height = _this2$_tooltip$getBo.height, width = _this2$_tooltip$getBo.width;
                let topIsPassed = event.clientY <= container.top + height + space;
                let top = event.pageY - height - space, left = event.pageX - width - space;
                ...
                _this2.css({top: top + "px", left: left + "px"});
            });
        };
    ...
    let DataVisualization = /*#__PURE__*/function () {
        function DataVisualization(_ref, map) {
            let scale = _ref.scale, values = _ref.values;
            this._scale = scale;
            this._values = values;
        ...
        let _proto = DataVisualization.prototype;
        ...
        _proto.setAttributes = function setAttributes(attrs) {
            for (let code in attrs) {
                if (this._map.regions[code]) this._map.regions[code].element.setStyle('fill', attrs[code]);
            }
        };
        _proto.getValue = function getValue(value) {
            let hex, color = '#';
     ...
     * ------------------------------------------------------------------------
     */
    let Map = /*#__PURE__*/function () {
        function Map(options) {
            let _this = this;
            if (options === void 0) options = {};
            // Merge the given options with the default options
            this.params = merge(Map.defaults, options, true);
            // Throw an error if the given map name doesn't match the map that was set in map file
            ...
            else window.addEventListener('DOMContentLoaded', () => { return _this._init(); });
        }
        let _proto = Map.prototype;
        ...
        _proto.addMarkers = function addMarkers(config) {
            if (Array.isArray(config)) return this._createMarkers(config, true);
        ...
        _proto.addLine = function addLine(from, to, style) {
            ...
            this._createLines([{from: from, to: to, style: style}], this._markers, true);
        };
        _proto.addLines = function addLines(config) {
            let uids = this._getLinesAsUids();
            if (!Array.isArray(config)) config = [config];
            this._createLines(config.filter((line) => { return !(uids.indexOf(getLineUid(line.from, line.to)) > -1); }), this._markers, true);
        };
        _proto.removeLines = function removeLines(lines) {
            let _this3 = this;
            if (Array.isArray(lines)) lines = lines.map((line) => { return getLineUid(line.from, line.to); });
        ...
        _proto._emit = function _emit(eventName, args) {
            for (let event in Events) {
                if (Events[event] === eventName && typeof this.params[event] === 'function') this.params[event].apply(this, args);
            }
        };
        ...
        _proto._getSelected = function _getSelected(type) {
            ...
            for (let key in this[type]) {
                if (this[type][key].element.isSelected) selected.push(key);
            }
            return selected;
        };
        _proto._setSelected = function _setSelected(type, keys) {
            ...
            keys.forEach(function (key) {
                if (_this5[type][key]) _this5[type][key].element.select(true);
            });
        };
        _proto._clearSelected = function _clearSelected(type) {
            let _this6 = this;
            this._getSelected(type).forEach((key) => { _this6[type][key].element.select(false); });
        };
     ...
     * ------------------------------------------------------------------------
     */
    let jsVectorMap = /*#__PURE__*/function () {
    ...
    let index = window.jsVectorMap = jsVectorMap;
    return index;
}));