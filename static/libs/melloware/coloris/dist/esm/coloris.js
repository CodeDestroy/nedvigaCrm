    ...
    */
    return function (window, document, Math) {
        ...
        function moveMarker(event) {
            let pointer = getPointerPosition(event), x = pointer.pageX - colorAreaDims.x;
        ...
        function moveMarkerOnKeydown(offsetX, offsetY) {
            let x = colorMarker.style.left.replace('px', '') * 1 + offsetX, y = colorMarker.style.top.replace('px', '') * 1 + offsetY;
            setMarkerPosition(x, y);
        }
        /**
        ...
        function init() {
            ...
            addListener(document, 'keydown', function (event) {
                ...
                if (key === 'Escape') closePicker(true); // Display focus rings when using the keyboard
        ...
        function getFocusableElements() {
            let controls = Array.from(picker.querySelectorAll('input, button'));
            return controls.filter((node) => { return !!node.offsetWidth; });
        }
        /**
        ...
        function addListener(context, type, selector, fn) {
            ...
            if (typeof selector === 'string') {
                ...
                // If the selector is not a string then it's a function in which case we need a regular event listener
            } else {
                fn = selector;
...
let _coloris = Coloris.coloris, _init = Coloris.init, _set = Coloris.set, _wrap = Coloris.wrap, _close = Coloris.close;
let _setInstance = Coloris.setInstance, _removeInstance = Coloris.removeInstance, _updatePosition = Coloris.updatePosition;
