...
(function () {
    ...
    const register$2 = editor => {
        ...
        registerOption('nonbreaking_force_tab', {
            processor: value => {
                if (isBoolean(value)) return {value: value ? 3 : 0, valid: true};
                else if (isNumber(value)) return {value, valid: true};
                return {valid: false, message: 'Must be a boolean or number.'};
            },
            default: false
        });
        registerOption('nonbreaking_wrap', {processor: 'boolean', default: true});
    };
    const getKeyboardSpaces = option('nonbreaking_force_tab'), wrapNbsps = option('nonbreaking_wrap');
    ...
    const register$1 = editor => { editor.addCommand('mceNonBreaking', () => { insertNbsp(editor, 1); }); };
    ...
    const register = editor => {
        ...
        editor.ui.registry.addButton('nonbreaking', {icon: 'non-breaking', tooltip: 'Nonbreaking space', onAction});
        editor.ui.registry.addMenuItem('nonbreaking', {icon: 'non-breaking', text: 'Nonbreaking space', onAction});
    };
    let Plugin = () => {
