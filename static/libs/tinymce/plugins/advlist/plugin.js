...
(function () {
    ...
    const register$1 = editor => {
        ...
        registerOption('advlist_number_styles', {processor: 'string[]', default: 'default,lower-alpha,lower-greek,lower-roman,upper-alpha,upper-roman'.split(',')});
        registerOption('advlist_bullet_styles', {processor: 'string[]', default: 'default,circle,square'.split(',')});
    };
    const getNumberStyles = option('advlist_number_styles'), getBulletStyles = option('advlist_bullet_styles');
    ...
    const isChildOfBody = (editor, elm) => { return editor.dom.isChildOf(elm, editor.getBody()); };
    ...
    const getSelectedStyleType = editor => {
        ...
        return Optional.from(editor.dom.getStyle(listElm, 'listStyleType'));
    };
    ...
    const setNodeChangeHandler = (editor, nodeChangeHandler) => {
        ...
        nodeChangeHandler({parents: editor.dom.getParents(initialNode), element: initialNode});
        ...
        editor.ui.registry.addSplitButton(id, {
            ...
            fetch: callback => {
                const items = global.map(styles, styleValue => {
                    ...
                    return {type: 'choiceitem', value: itemValue, icon: 'list-' + iconStyle + '-' + iconName, text: displayText};
                });
                callback(items);
            },
            ...
            select: value => { return getSelectedStyleType(editor).map(listStyle => value === listStyle).getOr(false); },
