...
(function () {
    ...
    let global$1 = tinymce.util.Tools.resolve('tinymce.dom.RangeUtils');
    let global = tinymce.util.Tools.resolve('tinymce.util.Tools');
    const option = name => editor => editor.options.get(name);
    const register$2 = editor => {
        const registerOption = editor.options.register;
        registerOption('allow_html_in_named_anchor', {processor: 'boolean', default: false});
    };
    const allowHtmlInNamedAnchor = option('allow_html_in_named_anchor');
    const namedAnchorSelector = 'a:not([href])', isEmptyString = str => !str;
    const getIdFromAnchor = elm => { return elm.getAttribute('id') || elm.getAttribute('name') || ''; };
    ...
    const removeEmptyNamedAnchorsInSelection = editor => {
        ...
        global$1(dom).walk(editor.selection.getRng(), nodes => {
            global.each(nodes, node => {
                if (isEmptyNamedAnchor(node)) dom.remove(node, false);
            });
        });
    };
    ...
    const getId = editor => {
        ...
        if (anchor) return getIdFromAnchor(anchor);
        return '';
    };
    const createAnchor = (editor, id) => {
        editor.undoManager.transact(() => {
            if (!allowHtmlInNamedAnchor(editor)) editor.selection.collapse(true);
            if (editor.selection.isCollapsed()) editor.insertContent(editor.dom.createHTML('a', {id}));
            else {
    ...
    const insert = (editor, id) => {
        const anchor = getNamedAnchor(editor);
        if (anchor) updateAnchor(editor, id, anchor);
        else createAnchor(editor, id);
        editor.focus();
    };
    ...
    const open = editor => {
        ...
        editor.windowManager.open({
            ...
            onSubmit: api => {
                if (insertAnchor(editor, api.getData().id)) api.close();
            }
        });
    };
    const register$1 = editor => {
        editor.addCommand('mceAnchor', () => { open(editor); });
    };
    ...
    const setContentEditable = state => nodes => {
        for (let i = 0; i < nodes.length; i++) {
            const node = nodes[i];
            if (isEmptyNamedAnchorNode(node)) node.attr('contenteditable', state);
        }
    };
    const setup = editor => {
        editor.on('PreInit', () => {
    ...
    const registerFormats = editor => {
        editor.formatter.register('namedAnchor', {
            ...
            deep: true,
            attributes: {id: '%value'},
            onmatch: (node, _fmt, _itemName) => { return isNamedAnchor(node); }
        });
    };
    const register = editor => {
        ...
        editor.ui.registry.addMenuItem('anchor', {icon: 'bookmark', text: 'Anchor...', onAction});
    };
    let Plugin = () => {
        global$2.add('anchor', editor => {
            ...
            register$1(editor);
            register(editor);
            editor.on('PreInit', () => { registerFormats(editor); });
        });
    };
    Plugin();
})();