...
(function () {
    ...
    let global$2 = tinymce.util.Tools.resolve('tinymce.PluginManager');
    ...
    const isNull = eq$1(null), isBoolean = isSimpleType('boolean'), isUndefined = eq$1(undefined);
    ...
    const never = constant(false), always = constant(true);
    class Optional {
    ...
    const findMap = (arr, f) => {
        for (let i = 0; i < arr.length; i++) {
            if (f(arr[i], i).isSome()) return f(arr[i], i);
        }
        return Optional.none();
    };
    const keys = Object.keys;
    const each = (obj, f) => {
        for (let k = 0, len = keys(obj).length; k < len; k++) {
    ...
    const DOCUMENT = 9, DOCUMENT_FRAGMENT = 11, ELEMENT = 1, TEXT = 3;
    ...
    const parents = (element, isRoot) => {
        ...
        while (dom.parentNode !== null && dom.parentNode !== undefined) {
            const rawParent = dom.parentNode, p = SugarElement.fromDom(rawParent);
            ret.push(p);
    ...
    const remove = (element, key) => { element.dom.removeAttribute(key); };
    ...
    const setAll = (element, css) => {
        each(css, (v, k) => { internalSet(element.dom, k, v); });
    };
    const get$2 = (element, property) => {
        const r = window.getComputedStyle(element.dom).getPropertyValue(property);
    ...
    const fromRawEvent = rawEvent => {
        ...
        const stop = () => rawEvent.stopPropagation(), prevent = () => rawEvent.preventDefault();
        return mkEvent(target, rawEvent.clientX, rawEvent.clientY, stop, prevent, compose(prevent, stop), rawEvent);
    };
    const handle = (filter, handler) => rawEvent => {
        if (filter(rawEvent)) handler(fromRawEvent(rawEvent));
    };
    const binder = (element, event, filter, handler, useCapture) => {
        element.dom.addEventListener(event, handle(filter, handler), useCapture);
        return {unbind: curry(unbind, element, event, handle(filter, handler), useCapture)};
    };
    ...
    const Version = {nu: nu$2, detect: detect$3, unknown: unknown$2};
    ...
    const checkContains = target => {
        return uastring => { return contains(uastring, target); };
    };
    const browsers = [{
        name: 'Edge',
        versionRegexes: [/.*?edge\/ ?([0-9]+)\.([0-9]+)$/],
        search: uastring => { return contains(uastring, 'edge/') && contains(uastring, 'chrome') && contains(uastring, 'safari') && contains(uastring, 'applewebkit'); }
    }, {
        name: 'Chromium',
        brand: 'Chromium',
    ...
    const windows = 'Windows', ios = 'iOS', android = 'Android', linux = 'Linux';
    const macos = 'macOS', solaris = 'Solaris', freebsd = 'FreeBSD', chromeos = 'ChromeOS';
    ...
    const get = _win => {
        ...
        if (detect().browser.isFirefox()) return Optional.none();
        return Optional.from(win.visualViewport);
    };
    const bounds = (x, y, width, height) => ({x, y, width, height, right: x + width, bottom: y + height});
    const getBounds = _win => {
        const win = _win === undefined ? window : _win, doc = win.document;
        const scroll = get$1(SugarElement.fromDom(doc));
        return get(win).fold(() => {
            const html = win.document.documentElement, width = html.clientWidth;
    ...
    const register$2 = editor => { editor.options.register('fullscreen_native', {processor: 'boolean', default: false}); };
    ...
    const getFullscreenElement = root => {
        if (root.fullscreenElement !== undefined) return root.fullscreenElement;
        else if (root.msFullscreenElement !== undefined) return root.msFullscreenElement;
        else if (root.webkitFullscreenElement !== undefined) return root.webkitFullscreenElement;
        return null;
    };
    const getFullscreenchangeEventName = () => {
        if (document.fullscreenElement !== undefined) return 'fullscreenchange';
        else if (document.msFullscreenElement !== undefined) return 'MSFullscreenChange';
        else if (document.webkitFullscreenElement !== undefined) return 'webkitfullscreenchange';
    ...
    const attr = 'data-ephox-mobile-fullscreen-style', siblingStyles = 'display:none!important;';
    ...
    const bgFallback = 'background-color:rgb(255,255,255)!important;', isAndroid = global.os.isAndroid();
    ...
    const clobberStyles = (dom, container, editorBody) => {
        const gatherSiblings = element => { return siblings(element, '*:not(.tox-silver-sink)'); };
        const clobber = clobberStyle => element => {
            const styles = get$3(element, 'style'), backup = styles === undefined ? 'no-styles' : styles.trim();
            if (backup === clobberStyle) return;
            set(element, attr, backup);
            setAll(element, dom.parseStyle(clobberStyle));
        };
        const ancestors$1 = ancestors(container, '*'), siblings$1 = bind$3(ancestors$1, gatherSiblings);
    ...
    const restoreStyles = dom => {
        ...
        each$1(clobberedEls, element => {
            ...
            if (restore && restore !== 'no-styles') setAll(element, dom.parseStyle(restore));
            else remove(element, 'style');
            remove(element, attr);
        });
    };
    const DOM = global$1.DOM, getScrollPos = () => getBounds(window);
    const setScrollPos = pos => window.scrollTo(pos.x, pos.y);
    const viewportUpdate = get().fold(() => ({bind: noop, unbind: noop}), visualViewport => {
        const editorContainer = value(), resizeBinder = unbindable(), scrollBinder = unbindable();
        const refreshScroll = () => {
        ...
        return {bind: bind$1, unbind};
    });
    const toggleFullscreen = (editor, fullscreenState) => {
        const body = document.body, documentElement = document.documentElement;
        const editorContainer = editor.getContainer(), editorContainerS = SugarElement.fromDom(editorContainer);
        const fullscreenRoot = getFullscreenRoot(editor), fullscreenInfo = fullscreenState.get();
        const editorBody = SugarElement.fromDom(editor.getBody()), isTouch = global.deviceType.isTouch();
        const editorContainerStyle = editorContainer.style, iframe = editor.iframeElement;
        ...
        const cleanup = () => {
            if (isTouch) restoreStyles(editor.dom);
        ...
        if (!fullscreenInfo) {
            const fullscreenChangeHandler = bind$1(owner(fullscreenRoot), getFullscreenchangeEventName(), _evt => {
                if (getFullscreenNative(editor)) {
                    if (!isFullscreenElement(fullscreenRoot) && fullscreenState.get() !== null) toggleFullscreen(editor, fullscreenState);
                }
            });
            ...
            if (isTouch) clobberStyles(editor.dom, editorContainerS, editorBody);
            ...
            if (getFullscreenNative(editor)) requestFullscreen(fullscreenRoot);
            fireFullscreenStateChanged(editor, true);
        } else {
            fullscreenInfo.fullscreenChangeHandler.unbind();
            if (getFullscreenNative(editor) && isFullscreenElement(fullscreenRoot)) exitFullscreen(owner(fullscreenRoot));
    ...
    let Plugin = () => {
        global$2.add('fullscreen', editor => {
            ...
            if (editor.inline) return get$5(fullscreenState);
            register$2(editor);
