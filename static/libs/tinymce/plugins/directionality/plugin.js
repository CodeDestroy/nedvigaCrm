...
(function () {
    ...
    const isString = isType$1('string'), isBoolean = isSimpleType('boolean');
    const isNullable = a => a === null || a === undefined, isNonNullable = a => !isNullable(a);
    const isFunction = isSimpleType('function'), isNumber = isSimpleType('number'), compose1 = (fbc, fab) => a => fbc(fab(a));
    ...
    const fromTag = (tag, scope) => {
        ...
        return fromDom(doc.createElement(tag));
    };
    const fromText = (text, scope) => {
        const doc = scope || document;
        return fromDom(doc.createTextNode(text));
    };
    const fromDom = node => {
    ...
    const name = element => { return element.dom.nodeName.toLowerCase(); }, type = element => element.dom.nodeType;
    const isType = t => element => type(element) === t, isElement = isType(ELEMENT), isText = isType(TEXT);
    const isDocument = isType(DOCUMENT), isDocumentFragment = isType(DOCUMENT_FRAGMENT);
    ...
    const get = (element, property) => {
        const dom = element.dom, styles = window.getComputedStyle(dom);
        ...
        if (selectedBlocks.length > 0) {
            each(selectedBlocks, block => {
                ...
                normalizedBlockParent.each(parent => {
                    ...
                    if (parentDirection !== dir) set(normalizedBlock, 'dir', dir);
                    else if (getDirection(normalizedBlock) !== dir) remove(normalizedBlock, 'dir');
    ...
    const register$1 = editor => {
        editor.addCommand('mceDirectionLTR', () => { setDir(editor, 'ltr'); });
        editor.addCommand('mceDirectionRTL', () => { setDir(editor, 'rtl'); });
    };
    const getNodeChangeHandler = (editor, dir) => api => {
        const nodeChangeHandler = e => { api.setActive(getDirection(SugarElement.fromDom(e.element)) === dir); };
