...
(function () {
    ...
    const prismjs = function (global, module, exports) {
        ...
        let Prism = function (_self) {
            let lang = /(?:^|\s)lang(?:uage)?-([\w-]+)(?=\s|$)/i, uniqueId = 0, plainTextGrammar = {};
            let _ = {
                ...
                util: {
                    ...
                    isActive: function (element, className, defaultActivation) {
                        ...
                        while (element) {
                            ...
                            if (element.classList.contains(no)) return false;
                            element = element.parentElement;
                        }
                    ...
                    DFS: function DFS(o, callback, type, visited) {
                        ...
                        for (let i in o) {
                            if (o.hasOwnProperty(i)) {
                                ...
                                let property = o[i], propertyType = _.util.type(property);
                ...
                highlightAllUnder: function (container, async, callback) {
                    let env = {callback: callback, container: container, selector: 'code[class*="language-"], [class*="language-"] code, code[class*="lang-"], [class*="lang-"] code'};
                ...
                highlightElement: function (element, async, callback) {
                    let language = _.util.getLanguage(element), grammar = _.languages[language];
                    _.util.setLanguage(element, language);
                    let parent = element.parentElement;
                    if (parent && parent.nodeName.toLowerCase() === 'pre') _.util.setLanguage(parent, language);
                    let code = element.textContent, env = {element: element, language: language, grammar: grammar, code: code};
                ...
                highlight: function (text, grammar, language) {
                    let env = {code: text, grammar: grammar, language: language};
            ...
            function matchGrammar(text, tokenList, grammar, startNode, startPos, rematch) {
                for (let token in grammar) {
                    ...
                    for (let j = 0; j < patterns.length; ++j) {
                        ...
                        let patternObj = patterns[j], inside = patternObj.inside, lookbehind = !!patternObj.lookbehind;
                        let greedy = !!patternObj.greedy, alias = patternObj.alias;
                        ...
                        for (let currentNode = startNode.next, pos = startPos; currentNode !== tokenList.tail; pos += currentNode.value.length, currentNode = currentNode.next) {
                            ...
                            let removeCount = 1, match;
                            if (greedy) {
                                match = matchPattern(pattern, pos, text, lookbehind);
                                if (!match || match.index >= text.length) break;
                                let from = match.index, to = match.index + match[0].length, p = pos;
                            ...
                            let from = match.index, matchStr = match[0], before = str.slice(0, from);
                            let after = str.slice(from + matchStr.length), reach = pos + str.length;
                            ...
                            if (removeCount > 1) {
                                let nestedRematch = {cause: token + ',' + j, reach: reach};
            ...
            function addAfter(list, node, value) {
                let next = node.next, newNode = {value: value, prev: node, next: next};
                node.next = newNode;
                next.prev = newNode;
                list.length++;
                return newNode;
            ...
            if (!_.manual) {
                ...
                if (readyState === 'loading' || readyState === 'interactive' && script && script.defer) document.addEventListener('DOMContentLoaded', highlightAutomaticallyCallback);
                    ...
                    if (window.requestAnimationFrame) window.requestAnimationFrame(highlightAutomaticallyCallback);
            ...
            Object.defineProperties(Prism.languages['markup-templating'] = {}, {
                buildPlaceholders: {
                    value: function (env, language, placeholderPattern, replaceFilter) {
                        ...
                        env.code = env.code.replace(placeholderPattern, function (match) {
                            ...
                            let i = tokenStack.length, placeholder;
                ...
                tokenizePlaceholders: {
                    value: function (env, language) {
                        ...
                        let j = 0, keys = Object.keys(env.tokenStack);
                        function walkTokens(tokens) {
                            for (let i = 0; i < tokens.length; i++) {
                                if (j >= keys.length) break;
                                let token = tokens[i];
                                if (typeof token === 'string' || token.content && typeof token.content === 'string') {
                                    let k = keys[j], t = env.tokenStack[k];
        ...
        Prism.languages.c = Prism.languages.extend('clike', {
            'comment': {pattern: /\/\/(?:[^\r\n\\]|\\(?:\r\n?|\n|(?![\r\n])))*|\/\*[\s\S]*?(?:\*\/|$)/, greedy: true},
            'string': {pattern: /"(?:\\(?:\r\n|[\s\S])|[^"\\\r\n])*"/, greedy: true},
            'class-name': {pattern: /(\b(?:enum|struct)\s+(?:__attribute__\s*\(\([\s\S]*?\)\)\s*)?)\w+|\b[a-z]\w*_t\b/, lookbehind: true},
        ...
        (function (Prism) {
            ...
            let modName = /\b(?!<keyword>)\w+(?:\s*\.\s*\w+)*\b/.source.replace(/<keyword>/g, function () {
            ...
            Prism.languages.insertBefore('cpp', 'keyword', {
                'generic-function': {
                    ...
                    inside: {
                        'function': /^\w+/,
                        'generic': {pattern: /<[\s\S]+/, alias: 'class-name', inside: Prism.languages.cpp}
                    }
                }
            });
            Prism.languages.insertBefore('cpp', 'operator', {'double-colon': {pattern: /::/, alias: 'punctuation'}});
        ...
        (function (Prism) {
            function replace(pattern, replacements) {
                return pattern.replace(/<<(\d+)>>/g, function (m, index) { return '(?:' + replacements[+index] + ')'; });
            }
            function re(pattern, replacements, flags) { return RegExp(replace(pattern, replacements), flags || ''); }
            ...
            let keywordKinds = {
                type: 'bool byte char decimal double dynamic float int long object sbyte short string uint ulong ushort let void',
                typeDeclaration: 'class enum interface record struct',
                contextual: 'add alias and ascending async await by descending from(?=\\s*(?:\\w|$)) get global group into init(?=\\s*;) join let nameof not notnull on or orderby partial remove select set unmanaged value when where with(?=\\s*{)',
                other: 'abstract as base break case catch checked const continue default delegate do else event explicit extern finally fixed for foreach goto if implicit in internal is lock namespace new null operator out override params private protected public readonly ref return sealed sizeof stackalloc static switch this throw try typeof unchecked unsafe using virtual volatile while yield'
            };
            function keywordsToPattern(words) { return '\\b(?:' + words.trim().replace(/ /g, '|') + ')\\b'; }
            ...
            let generic = nested(/<(?:[^<>;=+\-*/%&|^]|<<self>>)*>/.source, 2);
            ...
            let typeExpressionWithoutTuple = replace(/<<0>>(?:\s*(?:\?\s*)?<<1>>)*(?:\s*\?)?/.source, [
                identifier,
                array
            ]);
            let tupleElement = replace(/[^,()<>[\];=+\-*/%&|^]|<<0>>|<<1>>|<<2>>/.source, [
                generic,
                nestedRound,
                array
            ]);
            let tuple = replace(/\(<<0>>+(?:,<<0>>+)+\)/.source, [tupleElement]);
            let typeExpression = replace(/(?:<<0>>|<<1>>)(?:\s*(?:\?\s*)?<<2>>)*(?:\s*\?)?/.source, [
                tuple,
                identifier,
                array
            ]);
            let typeInside = {'keyword': keywords, 'punctuation': /[<>()?,.:[\]]/};
            ...
            Prism.languages.csharp = Prism.languages.extend('clike', {
                ...
                'class-name': [
                    {pattern: re(/(\busing\s+static\s+)<<0>>(?=\s*;)/.source, [identifier]), lookbehind: true, inside: typeInside},
                    {
            ...
            Prism.languages.insertBefore('csharp', 'number', {'range': {pattern: /\.\./, alias: 'operator'}});
            Prism.languages.insertBefore('csharp', 'punctuation', {
                'named-parameter': {pattern: re(/([(,]\s*)<<0>>(?=\s*:)/.source, [name]), lookbehind: true, alias: 'punctuation'}
            });
            ...
            Prism.languages.insertBefore('csharp', 'class-name', {
                'attribute': {
                    ...
                    inside: {
                        ...
                        'attribute-arguments': {pattern: re(/\(<<0>>*\)/.source, [roundExpression]), inside: Prism.languages.csharp},
                ...
                return {
                    'interpolation': {
                        ...
                        inside: {
                            ...
                            'punctuation': /^\{|\}$/,
                            'expression': {pattern: /[\s\S]+/, alias: 'language-csharp', inside: Prism.languages.csharp}
                        }
                    },
                    'string': /[\s\S]+/
                };
        ...
        (function (Prism) {
            ...
            Prism.languages.css = {
                ...
                'property': {pattern: /(^|[^-\w\xA0-\uFFFF])(?!\s)[-_a-z\xA0-\uFFFF](?:(?!\s)[-\w\xA0-\uFFFF])*(?=\s*:)/i, lookbehind: true},
        ...
        (function (Prism) {
            ...
            Prism.languages.java = Prism.languages.extend('clike', {
                ...
                'keyword': keywords,
                'function': [Prism.languages.clike.function, {pattern: /(::\s*)[a-z_]\w*/, lookbehind: true}],
        ...
        Object.defineProperty(Prism.languages.markup.tag, 'addInlined', {
            value: function addInlined(tagName, lang) {
                ...
                let inside = {'included-cdata': {pattern: /<!\[CDATA\[[\s\S]*?\]\]>/i, inside: includedCdataInside}};
        ...
        (function (Prism) {
            ...
            Prism.languages.php = {
                ...
                'constant': constant,
                'function': {pattern: /(^|[^\\\w])\\?[a-z_](?:[\w\\]*\w)?(?=\s*\()/i, lookbehind: true, inside: {'punctuation': /\\/}},
        ...
        (function (Prism) {
            ...
            Prism.languages.insertBefore('ruby', 'operator', {'double-colon': {pattern: /::/, alias: 'punctuation'}});
            let interpolation = {
                pattern: /((?:^|[^\\])(?:\\{2})*)#\{(?:[^{}]|\{[^{}]*\})*\}/,
                lookbehind: true,
                inside: {
                    'content': {pattern: /^(#\{)[\s\S]+(?=\}$)/, lookbehind: true, inside: Prism.languages.ruby},
                    'delimiter': {pattern: /^#\{|\}$/, alias: 'punctuation'}
                }
            };
            ...
            Prism.languages.insertBefore('ruby', 'keyword', {
                'regex-literal': [{
                    ...
                    greedy: true,
                    inside: {'interpolation': interpolation, 'regex': /[\s\S]+/}
                }, {
                    pattern: /(^|[^/])\/(?!\/)(?:\[[^\r\n\]]+\]|\\.|[^[/\\\r\n])+\/[egimnosux]{0,6}(?=\s*(?:$|[\r\n,.;})#]))/,
                    lookbehind: true,
                    greedy: true,
            ...
            Prism.languages.insertBefore('ruby', 'string', {
                'string-literal': [{
                    ...
                    greedy: true,
                    inside: {'interpolation': interpolation, 'string': /[\s\S]+/}
                }, {
                    pattern: /("|')(?:#\{[^}]+\}|#(?!\{)|\\(?:\r\n|[\s\S])|(?!\1)[^\\#\r\n])*\1/,
                    greedy: true,
                    inside: {'interpolation': interpolation, 'string': /[\s\S]+/}
                ...
                'command-literal': [{
                    ...
                    greedy: true,
                    inside: {'interpolation': interpolation, 'command': {pattern: /[\s\S]+/, alias: 'string'}}
                }, {
                    pattern: /`(?:#\{[^}]+\}|#(?!\{)|\\(?:\r\n|[\s\S])|[^\\`#\r\n])*`/,
                    greedy: true,
                    inside: {'interpolation': interpolation, 'command': {pattern: /[\s\S]+/, alias: 'string'}}
    ...
    const register$2 = editor => {
        ...
        registerOption('codesample_global_prismjs', {processor: 'boolean', default: false});
    };
    ...
    const getCurrentCode = editor => { return getSelectedCodeSample(editor).bind(n => Optional.from(n.textContent)).getOr(''); };
    ...
    const open = editor => {
        ...
        editor.windowManager.open({
            ...
            body: {
                type: 'panel',
                items: [
                    {type: 'selectbox', name: 'language', label: 'Language', items: languages},
            ...
            initialData: {language: currentLanguage, code: currentCode},
            onSubmit: api => {
    ...
    const register$1 = editor => {
        editor.addCommand('codesample', () => {
            if (editor.selection.isCollapsed() || isCodeSample(editor.selection.getNode())) open(editor);
    ...
    const isCodeSampleSelection = editor => { return editor.dom.is(editor.selection.getStart(), 'pre[class*="language-"]'); };
    const register = editor => {
        ...
        editor.ui.registry.addMenuItem('codesample', {text: 'Code sample...', icon: 'code-sample', onAction});
    };
    let Plugin = () => {
