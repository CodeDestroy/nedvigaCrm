...
(function () {
    ...
    const register$1 = editor => {
        ...
        registerOption('autoresize_overflow_padding', {processor: 'number', default: 1});
        registerOption('autoresize_bottom_margin', {processor: 'number', default: 50});
    };
    const getMinHeight = option('min_height'), getMaxHeight = option('max_height');
    ...
    const resize = (editor, oldSize, trigger, getExtraMarginBottom) => {
        let _a;
        const dom = editor.dom, doc = editor.getDoc();
        if (!doc) return;
        ...
        if (contentHeight + chromeHeight > minHeight) resizeHeight = contentHeight + chromeHeight;
        const maxHeight = getMaxHeight(editor);
        if (maxHeight && resizeHeight > maxHeight) {
            resizeHeight = maxHeight;
            toggleScrolling(editor, true);
        } else toggleScrolling(editor, false);
        if (resizeHeight !== oldSize.get()) {
            ...
            if (editor.hasFocus() && shouldScrollIntoView(trigger)) editor.selection.scrollIntoView();
            if ((global.browser.isSafari() || global.browser.isChromium()) && deltaSize < 0) resize(editor, oldSize, trigger, getExtraMarginBottom);
        }
    };
    const setup = (editor, oldSize) => {
        let getExtraMarginBottom = () => getAutoResizeBottomMargin(editor), resizeCounter, sizeAfterFirstResize;
        editor.on('init', e => {
            resizeCounter = 0;
            const overflowPadding = getAutoResizeOverflowPadding(editor), dom = editor.dom;
            ...
            } else dom.setStyles(editor.getBody(), {paddingLeft: overflowPadding, paddingRight: overflowPadding});
        ...
        editor.on('NodeChange SetContent keyup FullscreenStateChanged ResizeContent', e => {
            if (resizeCounter === 1) {
                ...
                resizeCounter += 1;
            } else resize(editor, oldSize, e, getExtraMarginBottom);
        });
    };
    const register = (editor, oldSize) => { editor.addCommand('mceAutoResize', () => { resize(editor, oldSize); }); };
    let Plugin = () => {
        global$1.add('autoresize', editor => {
            register$1(editor);
            if (!editor.options.isSet('resize')) editor.options.set('resize', false);
            if (!editor.inline) {
