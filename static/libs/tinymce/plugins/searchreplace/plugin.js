...
(function () {
    ...
    const isString = isType$1('string'), isArray = isType$1('array');
    const isBoolean = isSimpleType('boolean'), isNullable = a => a === null || a === undefined;
    const isNonNullable = a => !isNullable(a), isNumber = isSimpleType('number'), noop = () => {};
    ...
    let global$2 = tinymce.util.Tools.resolve('tinymce.Env'), global$1 = tinymce.util.Tools.resolve('tinymce.util.Tools');
    const nativeSlice = Array.prototype.slice, nativePush = Array.prototype.push;
    const map = (xs, f) => {
    ...
    const hasOwnProperty = Object.hasOwnProperty, has = (obj, key) => hasOwnProperty.call(obj, key);
    typeof window !== 'undefined' ? window : Function('return this;')();
    const DOCUMENT = 9, DOCUMENT_FRAGMENT = 11, ELEMENT = 1, TEXT = 3;
    const type = element => element.dom.nodeType, isType = t => element => type(element) === t, isText$1 = isType(TEXT);
    ...
    const api = NodeValue(isText$1, 'text'), get$1 = element => api.get(element);
    const compareDocumentPosition = (a, b, match) => { return (a.compareDocumentPosition(b) & match) !== 0; };
    const documentPositionPreceding = (a, b) => { return compareDocumentPosition(a, b, Node.DOCUMENT_POSITION_PRECEDING); };
    const descendants = (scope, selector) => all(selector, scope);
    let global = tinymce.util.Tools.resolve('tinymce.dom.TreeWalker');
    ...
    const isText = node => node.nodeType === 3, nuSection = () => ({sOffset: 0, fOffset: 0, elements: []});
    ...
    const collect = (dom, rootNode, startNode, endNode, callbacks, skipStart = true) => {
        const walker = new global(startNode, rootNode), sections = [];
    ...
    const collectRangeSections = (dom, rng) => {
        const start = toLeaf(rng.startContainer, rng.startOffset), startNode = start.element.dom;
        const end = toLeaf(rng.endContainer, rng.endOffset), endNode = end.element.dom;
        return collect(dom, rng.commonAncestorContainer, startNode, endNode, {
            ...
            cef: node => {
                const sections = bind(descendants(SugarElement.fromDom(node), '*[contenteditable=true]'), e => {
                    return collect(dom, e.dom, e.dom);
                });
    ...
    const find$2 = (text, pattern, start = 0, finish = text.length) => {
        ...
        while (match = regex.exec(text)) {
            ...
            results.push({start: matchStart, finish: matchFinish});
    ...
    const extract = (elements, matches) => {
        const nodePositions = foldl(elements, (acc, element) => {
            const content = get$1(element), start = acc.last, finish = start + content.length;
            const positions = bind(matches, (match, matchIdx) => {
                if (match.start < finish && match.finish > start) return [{element, start: Math.max(start, match.start) - start, finish: Math.min(finish, match.finish) - start, matchId: matchIdx}];
                return [];
            });
            return {results: acc.results.concat(positions), last: finish};
    ...
    const find$1 = (pattern, sections) => bind(sections, section => {
        const content = map(section.elements, get$1).join('');
        const positions = find$2(content, pattern, section.sOffset, content.length - section.fOffset);
        return extract(section.elements, positions);
    });
        ...
        eachr(matches, (match, idx) => {
            eachr(match, pos => {
                ...
                if (textNode.length === pos.finish && pos.start === 0) wrap(pos.element, wrapper);
                    ...
                    wrap(SugarElement.fromDom(textNode.splitText(pos.start)), wrapper);
                }
            });
        });
    };
    const findAndMark = (dom, pattern, node, replacementNode) => {
        const matches = find$1(pattern, fromNode(dom, node));
    ...
    const getElmIndex = elm => { return elm.getAttribute('data-mce-index'); };
    const markAllMatches = (editor, currentSearchState, pattern, inSelection) => {
        ...
        return findAndMark(editor.dom, pattern, editor.getBody(), marker);
    };
    const unwrap = node => {
        let _a;
        if (node.firstChild) node.parentNode.insertBefore(node.firstChild, node);
    ...
    const moveSelection = (editor, currentSearchState, forward) => {
        ...
        if (forward) {
            ...
            if (testIndex - 1 === -1) testIndex = searchState.count - 1;
            else testIndex--;
        }
        editor.dom.removeClass(findSpansByIndex(editor, searchState.index), 'mce-match-marker-selected');
        const spans = findSpansByIndex(editor, testIndex);
        if (spans.length) {
            editor.dom.addClass(findSpansByIndex(editor, testIndex), 'mce-match-marker-selected');
    ...
    const removeNode = (dom, node) => {
        dom.remove(node);
        if (node.parentNode && dom.isEmpty(node.parentNode)) dom.remove(node.parentNode);
    };
    ...
    const find = (editor, currentSearchState, text, matchCase, wholeWord, inSelection) => {
        const pattern = {
            regex: new RegExp(escapeSearchText(text, wholeWord), matchCase ? 'g' : 'gi'),
            matchIndex: 1
        };
        const count = markAllMatches(editor, currentSearchState, pattern, inSelection);
        if (global$2.browser.isSafari()) editor.selection.setRng(editor.selection.getRng(), editor.selection.isForward());
        if (count) {
            const newIndex = moveSelection(editor, currentSearchState, true);
            currentSearchState.set({index: newIndex, count, text, matchCase, wholeWord, inSelection});
        }
        return count;
    };
    ...
    const replace = (editor, currentSearchState, text, forward, all) => {
        const currentIndex = currentSearchState.get().index;
        let currentMatchIndex, nextIndex = currentIndex;
        forward = forward !== false;
        const node = editor.getBody(), nodes = global$1.grep(global$1.toArray(node.getElementsByTagName('span')), isMatchSpan);
        for (let i = 0; i < nodes.length; i++) {
            const nodeIndex = getElmIndex(nodes[i]);
            let matchIndex = currentMatchIndex = parseInt(nodeIndex, 10);
            if (all || matchIndex === currentSearchState.get().index) {
                if (text.length) {
        ...
        currentSearchState.set({...currentSearchState.get(), count: all ? 0 : currentSearchState.get().count - 1, index: nextIndex});
    ...
    const done = (editor, currentSearchState, keepEditorSelection) => {
        ...
        const searchState = currentSearchState.get(), nodes = global$1.toArray(editor.getBody().getElementsByTagName('span'));
        for (let i = 0; i < nodes.length; i++) {
            const nodeIndex = getElmIndex(nodes[i]);
        ...
        currentSearchState.set({...searchState, index: -1, count: 0, text: ''});
    ...
    const get = (editor, currentState) => {
        const done$1 = keepEditorSelection => { return done(editor, currentState, keepEditorSelection); };
        const find$1 = (text, matchCase, wholeWord, inSelection = false) => { return find(editor, currentState, text, matchCase, wholeWord, inSelection); };
        const next$1 = () => { return next(editor, currentState); };
        const prev$1 = () => { return prev(editor, currentState); };
        const replace$1 = (text, forward, all) => { return replace(editor, currentState, text, forward, all); };
        return {done: done$1, find: find$1, next: next$1, prev: prev$1, replace: replace$1};
    };
    ...
    const open = (editor, currentSearchState) => {
        ...
        const updateSearchState = api => { currentSearchState.set({...currentSearchState.get(), matchCase: api.getData().matchcase, wholeWord: api.getData().wholewords, inSelection: api.getData().inselection}); };
        const disableAll = (api, disable) => {
            const toggle = name => api.setEnabled(name, !disable);
            each(['replace', 'replaceall', 'prev', 'next'], toggle);
        };
        const notFoundAlert = api => { api.redial(getDialogSpec(true, api.getData())); };
        ...
        const doFind = api => {
            const data = api.getData(), last = currentSearchState.get();
            ...
            if (last.text === data.findtext && last.matchCase === data.matchcase && last.wholeWord === data.wholewords) next(editor, currentSearchState);
                ...
                if (count <= 0) notFoundAlert(api);
                disableAll(api, count === 0);
            }
            updateButtonStates(api);
        };
        const initialState = currentSearchState.get();
        const initialData = {findtext: selectedText, replacetext: '', wholewords: initialState.wholeWord, matchcase: initialState.matchCase, inselection: initialState.inSelection};
        const getPanelItems = error => {
            const items = [{
                type: 'bar',
                items: [
                    {type: 'input', name: 'findtext', placeholder: 'Find', maximized: true, inputMode: 'search'},
                    {type: 'button', name: 'prev', text: 'Previous', icon: 'action-prev', enabled: false, borderless: true},
                    {type: 'button', name: 'next', text: 'Next', icon: 'action-next', enabled: false, borderless: true}
                ]
            }, {type: 'input', name: 'replacetext', placeholder: 'Replace with', inputMode: 'search'}];
            if (error) items.push({type: 'alertbanner', level: 'error', text: 'Could not find the specified string.', icon: 'warning'});
            return items;
        };
        const getDialogSpec = (showNoMatchesAlertBanner, initialData) => ({
            title: 'Find and Replace',
            size: 'normal',
            body: {type: 'panel', items: getPanelItems(showNoMatchesAlertBanner)},
            buttons: [{
                type: 'menu',
                name: 'options',
                icon: 'preferences',
                ...
                type: 'custom', name: 'find', text: 'Find', primary: true
            }, {
                type: 'custom', name: 'replace', text: 'Replace', enabled: false
            }, {
                type: 'custom', name: 'replaceall', text: 'Replace all', enabled: false
            }],
            initialData,
            onChange: (api, details) => {
                if (showNoMatchesAlertBanner) api.redial(getDialogSpec(false, api.getData()));
    ...
    const register$1 = (editor, currentSearchState) => {
        editor.addCommand('SearchReplace', () => { open(editor, currentSearchState); });
    };
    const showDialog = (editor, currentSearchState) => () => { open(editor, currentSearchState); };
    const register = (editor, currentSearchState) => {
        editor.ui.registry.addMenuItem('searchreplace', {text: 'Find and replace...', shortcut: 'Meta+F', onAction: showDialog(editor, currentSearchState), icon: 'search'});
        editor.ui.registry.addButton('searchreplace', {tooltip: 'Find and replace', onAction: showDialog(editor, currentSearchState), icon: 'search'});
