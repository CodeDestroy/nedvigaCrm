...
(function () {
    'use strict';
    let global$5 = tinymce.util.Tools.resolve('tinymce.PluginManager');
    ...
    const eq = t => a => t === a, isString = isType('string'), isObject = isType('object');
    ...
    const register$1 = editor => {
        ...
        registerOption('link_assume_external_targets', {
            processor: value => {
                ...
                if (valid) {
                    if (value === true) return {value: 1, valid};
                    else if (value === 'http' || value === 'https') return {value, valid};
        ...
        registerOption('link_context_toolbar', {processor: 'boolean', default: false});
        ...
        registerOption('link_target_list', {processor: value => isBoolean(value) || isArrayOf(value, isObject), default: true});
        registerOption('link_rel_list', {processor: 'object[]', default: []});
        registerOption('link_class_list', {processor: 'object[]', default: []});
        registerOption('link_title', {processor: 'boolean', default: true});
        registerOption('allow_unsafe_link_target', {processor: 'boolean', default: false});
        registerOption('link_quicklink', {processor: 'boolean', default: false});
    };
    const assumeExternalTargets = option('link_assume_external_targets');
    const hasContextToolbar = option('link_context_toolbar'), getLinkList = option('link_list');
    const getDefaultLinkTarget = option('link_default_target');
    const getDefaultLinkProtocol = option('link_default_protocol');
    const getTargetList = option('link_target_list'), getRelList = option('link_rel_list');
    const getLinkClassList = option('link_class_list'), shouldShowLinkTitle = option('link_title');
    const allowUnsafeLinkTarget = option('allow_unsafe_link_target');
    const useQuickLink = option('link_quicklink');
    let global$4 = tinymce.util.Tools.resolve('tinymce.util.Tools');
    ...
    const createUi = (name, label) => items => ({name, type: 'listbox', label, items});
    const ListOptions = {sanitize, sanitizeWith, createUi, getValue};
    ...
    let global$3 = tinymce.util.Tools.resolve('tinymce.dom.TreeWalker');
    ...
    const collectNodesInRange = (rng, predicate) => {
        ...
        const contents = rng.cloneContents(), firstChild = contents.firstChild;
        const walker = new global$3(firstChild, contents), elements = [];
    ...
    const applyRelTargetRules = (rel, isUnsafe) => {
        const rules = ['noopener'], rels = rel ? rel.split(/\s+/) : [];
    ...
    const isOnlyTextSelected = editor => {
        ...
        if (isInBlockAnchor) return false;
    ...
    const createLink = (editor, selectedElm, text, linkAttrs) => {
        if (isImageFigure(selectedElm)) linkImageFigure(editor.dom, selectedElm, linkAttrs);
        else {
            text.fold(() => {
    ...
    const unlinkSelection = editor => {
        const bookmark = editor.selection.getBookmark(), rng = editor.selection.getRng().cloneRange();
        const startAnchorElm = editor.dom.getParent(rng.startContainer, 'a[href]', editor.getBody());
        const endAnchorElm = editor.dom.getParent(rng.endContainer, 'a[href]', editor.getBody());
    ...
    const sanitizeData = (editor, data) => {
        ...
        return {...data, href: global$2.isDomSafe(href, 'a', uriOptions) ? href : ''};
    };
    const link = (editor, attachState, data) => {
        const sanitizedData = sanitizeData(editor, data);
        editor.hasPlugin('rtc', true) ? editor.execCommand('createlink', false, unwrapOptions(sanitizedData)) : linkDomMutation(editor, attachState, sanitizedData);
    };
    const unlink = editor => { editor.hasPlugin('rtc', true) ? editor.execCommand('unlink') : unlinkDomMutation(editor); };
    ...
    const getDelta = (persistentText, fieldName, catalog, data) => {
        const value = data[fieldName], hasPersistentText = persistentText.length > 0;
        return value !== undefined ? findTextByValue(value, catalog).map(i => ({
            url: {value: i.value, meta: {text: hasPersistentText ? persistentText : i.text, attach: noop}},
    ...
    const init = (initialData, linkCatalog) => {
        const persistentData = {text: initialData.text, title: initialData.title};
        ...
        const onUrlChange = data => {
            ...
            if (text.isSome() || title.isSome()) return Optional.some({...text.map(text => ({text})).getOr({}), ...title.map(title => ({title})).getOr({})});
            return Optional.none();
        };
        const onCatalogChange = (data, change) => { return getDelta(persistentData.text, change, findCatalog(linkCatalog, change).getOr([]), data); };
        const onChange = (getData, change) => {
            if (change.name === 'url') return onUrlChange(getData());
            else if (contains(['anchor', 'link'], change.name)) return onCatalogChange(getData(), change.name);
            else if (change.name === 'text' || change.name === 'title') persistentData[change.name] = getData()[change.name];
            return Optional.none();
        };
        return {onChange};
    };
    const DialogChanges = {init, getDelta};
    let global$1 = tinymce.util.Tools.resolve('tinymce.util.Delay');
    ...
    const getLinks = editor => {
        const extractor = item => editor.convertURL(item.value || item.url || '', 'href'), linkList = getLinkList(editor);
        return new Promise(resolve => {
            ...
            if (items.length > 0) return [{text: 'None', value: ''}].concat(items);
            return items;
        }));
    };
    ...
    const fallbacks = [{text: 'Current window', value: ''}, {text: 'New window', value: '_blank'}];
    ...
    const extractFromAnchor = (editor, anchor) => {
        ...
        return {url, text, title, target, rel, linkClass};
    };
    const collect = (editor, linkNode) => LinkListOptions.getLinks(editor).then(links => {
        const anchor = extractFromAnchor(editor, linkNode);
        return {
            anchor,
            catalogs: {targets: TargetOptions.getTargets(editor), rels: RelOptions.getRels(editor, anchor.target), classes: ClassListOptions.getClasses(editor), anchor: AnchorListOptions.getAnchors(editor), link: links},
            optNode: linkNode,
    ...
    const handleSubmit = (editor, info) => api => {
        ...
        const attachState = {href: data.url.value, attach: data.url.meta !== undefined && data.url.meta.attach ? data.url.meta.attach : noop};
    ...
    const getInitialData = (info, defaultTarget) => {
        ...
        return {
            url: {value: url, meta: {original: {value: url}}},
    ...
    const makeDialog = (settings, onSubmit, editor) => {
        const urlInput = [{name: 'url', type: 'urlinput', filetype: 'file', label: 'URL'}];
        const displayText = settings.anchor.text.map(() => ({name: 'text', type: 'input', label: 'Text to display'})).toArray();
        const titleText = settings.flags.titleEnabled ? [{name: 'title', type: 'input', label: 'Title'}] : [];
    ...
    let global = tinymce.util.Tools.resolve('tinymce.util.VK');
    ...
    const hasOnlyAltModifier = e => { return e.altKey === true && e.shiftKey === false && e.ctrlKey === false && e.metaKey === false; };
    ...
    const openDialog = editor => () => { editor.execCommand('mceLink', false, {dialog: true}); };
    const gotoSelectedLink = editor => () => { gotoLink(editor, getSelectedLink(editor)); };
    ...
    const setup = editor => {
        editor.addShortcut('Meta+K', '', () => { editor.execCommand('mceLink'); });
    };
    const setupButtons = editor => {
        editor.ui.registry.addToggleButton('link', {icon: 'link', tooltip: 'Insert/edit link', onAction: openDialog(editor), onSetup: toggleActiveState(editor)});
        editor.ui.registry.addButton('openlink', {icon: 'new-tab', tooltip: 'Open link', onAction: gotoSelectedLink(editor), onSetup: toggleEnabledState(editor)});
        editor.ui.registry.addButton('unlink', {icon: 'unlink', tooltip: 'Remove link', onAction: () => unlink(editor), onSetup: toggleUnlinkState(editor)});
    };
    const setupMenuItems = editor => {
        editor.ui.registry.addMenuItem('openlink', {text: 'Open link', icon: 'new-tab', onAction: gotoSelectedLink(editor), onSetup: toggleEnabledState(editor)});
        editor.ui.registry.addMenuItem('link', {icon: 'link', text: 'Link...', shortcut: 'Meta+K', onAction: openDialog(editor)});
        editor.ui.registry.addMenuItem('unlink', {icon: 'unlink', text: 'Remove link', onAction: () => unlink(editor), onSetup: toggleUnlinkState(editor)});
    };
    const setupContextMenu = editor => {
        const inLink = 'link unlink openlink', noLink = 'link';
    ...
    const setupContextToolbars = editor => {
        const collapseSelectionToEnd = editor => { editor.selection.collapse(false); };
        ...
        editor.ui.registry.addContextForm('quicklink', {
            ...
            commands: [{
                ...
                onAction: formApi => {
                    const value = formApi.getValue(), text = getLinkText(value), attachState = {href: value, attach: noop};
                    link(editor, attachState, {href: value, text, title: Optional.none(), rel: Optional.none(), target: Optional.none(), class: Optional.none()});
