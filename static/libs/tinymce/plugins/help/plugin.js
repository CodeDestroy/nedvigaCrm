...
(function () {
    ...
    const generate = prefix => {
        const date = new Date(), time = date.getTime(), random = Math.floor(Math.random() * 1000000000);
        unique++;
    ...
    const register$2 = (editor, dialogOpener) => { editor.addCommand('mceHelp', dialogOpener); };
    const option = name => editor => editor.options.get(name);
    const register$1 = editor => {
        const registerOption = editor.options.register;
        editor.options.register('help_tabs', {processor: 'array'});
    };
    const getHelpTabs = option('help_tabs'), getForcedPlugins = option('forced_plugins');
    const register = (editor, dialogOpener) => {
        editor.ui.registry.addButton('help', {icon: 'help', tooltip: 'Help', onAction: dialogOpener});
        editor.ui.registry.addMenuItem('help', {text: 'Help', icon: 'help', shortcut: 'Alt+0', onAction: dialogOpener});
    };
    ...
    const eq = t => a => t === a, isString = isType('string'), isUndefined = eq(undefined);
...
<p>When a tabbed dialog is opened, the first button in the tab menu is focused. Pressing tab will navigate to the first interactive component in that tab, and will cycle through the tab\u2019s components, the footer buttons, then back to the tab button. To switch to another tab, focus the tab button for the current tab, then use the arrow keys to cycle through the tab buttons.</p>`;
    const tab$3 = () => {
        return {name: 'keyboardnav', title: 'Keyboard Navigation', items: [{type: 'htmlpanel', presets: 'document', html: description}]};
    };
    let global$2 = tinymce.util.Tools.resolve('tinymce.Env');
    const convertText = source => {
        const isMac = global$2.os.isMacOS() || global$2.os.isiOS();
        const mac = {alt: '&#x2325;', ctrl: '&#x2303;', shift: '&#x21E7;', meta: '&#x2318;', access: '&#x2303;&#x2325;'};
        const other = {meta: 'Ctrl ', access: 'Shift + Alt '};
        const replace = isMac ? mac : other, shortcut = source.split('+');
    ...
    const tab$2 = () => {
        ...
        return {name: 'shortcuts', title: 'Handy Shortcuts', items: [{type: 'table', header: ['Action', 'Shortcut'], cells: shortcutList}]};
    };
    let global$1 = tinymce.util.Tools.resolve('tinymce.util.I18n');
    const urls = map([
    ...
    ], item => ({...item, type: item.type || 'opensource', slug: item.slug || item.key}));
    const tab$1 = editor => {
        const availablePlugins = () => {
            const premiumPlugins = filter(urls, ({type}) => { return type === 'premium'; });
        ...
        const identifyUnknownPlugin = (editor, key) => {
            ...
            if (isFunction(getMetadata)) return {name: getMetadata().name, html: makeLink(getMetadata())};
        ...
        const pluginLister = editor => {
            ...
            return '<p><b>' + global$1.translate(['Plugins installed ({0}):', pluginLis.length]) + '</b></p><ul>' + pluginLis.join('') + '</ul>';
        };
        const installedPlugins = editor => {
            if (editor == null) return '';
            return '<div data-mce-tabstop="1" tabindex="-1">' + pluginLister(editor) + '</div>';
        };
        return {name: 'plugins', title: 'Plugins', items: [{type: 'htmlpanel', presets: 'document', html: [installedPlugins(editor), availablePlugins()].join('')}]};
    };
    ...
    const tab = () => {
        ...
        return {name: 'versions', title: 'Version', items: [{type: 'htmlpanel', html: '<p>' + global$1.translate(['You are using {0}', changeLogLink]) + '</p>', presets: 'document'}]};
    };
    ...
    const getNamesFromTabs = tabs => {
        const names = keys(tabs), idx = names.indexOf('versions');
        if (idx !== -1) {
            names.splice(idx, 1);
            names.push('versions');
        }
    ...
    const parseCustomTabs = (editor, customTabs) => {
        const shortcuts = tab$2(), nav = tab$3(), plugins = tab$1(editor), versions = tab();
        const tabs = {[shortcuts.name]: shortcuts, [nav.name]: nav, [plugins.name]: plugins, [versions.name]: versions, ...customTabs.get()};
    ...
    const init = (editor, customTabs) => () => {
        ...
        const body = {type: 'tabpanel', tabs: dialogTabs};
        editor.windowManager.open({title: 'Help', size: 'normal', body, buttons: [{type: 'cancel', name: 'close', text: 'Close', primary: true}], initialData: {}});
    };
    let Plugin = () => {
        global$3.add('help', editor => {
            const customTabs = Cell({}), api = get$1(customTabs);
            register$1(editor);
