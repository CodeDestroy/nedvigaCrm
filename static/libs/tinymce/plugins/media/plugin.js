...
(function () {
    'use strict';
    let global$6 = tinymce.util.Tools.resolve('tinymce.PluginManager');
    ...
    const isType = type => value => typeOf(value) === type, isString = isType('string');
    const isObject = isType('object'), isArray = isType('array');
    ...
    const get$1 = (obj, key) => { return has(obj, key) ? Optional.from(obj[key]) : Optional.none(); };
    const has = (obj, key) => hasOwnProperty.call(obj, key), option = name => editor => editor.options.get(name);
    const register$2 = editor => {
        ...
        registerOption('media_live_embeds', {processor: 'boolean', default: true});
        registerOption('media_filter_html', {processor: 'boolean', default: true});
        registerOption('media_url_resolver', {processor: 'function'});
        registerOption('media_alt_source', {processor: 'boolean', default: true});
        registerOption('media_poster', {processor: 'boolean', default: true});
        registerOption('media_dimensions', {processor: 'boolean', default: true});
    };
    ...
    let global$5 = tinymce.util.Tools.resolve('tinymce.util.Tools');
    let global$4 = tinymce.util.Tools.resolve('tinymce.dom.DOMUtils');
    let global$3 = tinymce.util.Tools.resolve('tinymce.html.DomParser');
    const DOM$1 = global$4.DOM, trimPx = value => value.replace(/px$/, '');
    const getEphoxEmbedData = node => {
        const styles = node.attr('style') ? DOM$1.parseStyle(node.attr('style')) : {};
        return {
    ...
    const htmlToData = (html, schema) => {
        let data = {};
        const parser = global$3({validate: false, forced_root_block: false}, schema), rootNode = parser.parse(html);
        for (let node = rootNode; node; node = node.walk()) {
            if (node.type === 1) {
                ...
                if (node.attr('data-ephox-embed-iri')) {
                    ...
                    if (name === 'script') data = {type: 'script', source: node.attr('src')};
                    if (name === 'source') {
    ...
    let global$2 = tinymce.util.Tools.resolve('tinymce.html.Node');
    let global$1 = tinymce.util.Tools.resolve('tinymce.html.Serializer');
    const Parser = (schema, settings = {}) => global$3({forced_root_block: false, validate: false, allow_conditional_comments: true, ...settings}, schema);
    const DOM = global$4.DOM, addPx = value => /^[0-9.]+$/.test(value) ? value + 'px' : value;
    const updateEphoxEmbed = (data, node) => {
        const style = node.attr('style'), styleMap = style ? DOM.parseStyle(style) : {};
    ...
    const updateHtml = (html, data, updateAll, schema) => {
        let numSources = 0, sourceCount = 0;
    ...
    const getUrl = (pattern, url) => {
        ...
        let newUrl = getProtocol(url) + pattern.url;
    ...
    const getScriptHtml = data => { return '<script src="' + data.source + '"></script>'; };
    const dataToHtml = (editor, dataIn) => {
        ...
        if (data.embed) return updateHtml(data.embed, data, true, editor.schema);
            ...
            global$5.each(data, (value, key) => { data[key] = editor.dom.encode('' + value); });
            ...
            else if (data.type === 'script') return getScriptHtml(data);
    ...
    const setup$2 = editor => {
        ...
        editor.on('ObjectSelected', e => {
            ...
            if (objectType === 'script') e.preventDefault();
        });
        editor.on('ObjectResized', e => {
            ...
            if (target.getAttribute('data-mce-object')) {
                ...
                if (html) {
                    html = unescape(html);
                    target.setAttribute('data-mce-html', escape(updateHtml(html, {width: String(e.width), height: String(e.height)}, false, editor.schema)));
                }
            }
        });
    };
    ...
    const embedPromise = (data, dataToHtml, handler) => {
        return new Promise((res, rej) => {
            const wrappedResolve = response => {
                ...
                return res({url: data.source, html: response.html ? response.html : dataToHtml(data)});
            };
            if (cache[data.source]) wrappedResolve(cache[data.source]);
            else handler({url: data.source}, wrappedResolve, rej);
        });
    };
    ...
    const unwrap = (data, sourceInput) => {
        ...
        return {...get('source'), ...get('altsource'), ...get('poster'), ...get('embed'), ...getDimensions(data, metaData)};
    };
    const wrap = data => {
        const wrapped = {
            ...data,
    ...
    const getEditorData = editor => {
        ...
        return {embed: snippet, ...htmlToData(snippet, editor.schema)};
    };
    const addEmbedHtml = (api, editor) => response => {
        if (isString(response.url) && response.url.trim().length > 0) api.setData(wrap({...htmlToData(response.html, editor.schema), source: response.url, embed: response.html}));
    };
    ...
    const submitForm = (prevData, newData, editor) => {
        ...
        else getEmbedHtml(editor, newData).then(response => { handleInsert(editor, response.html); }).catch(handleError(editor));
    };
    const showDialog = editor => {
        const currentData = Cell(getEditorData(editor));
        const initialData = wrap(getEditorData(editor));
        ...
        const mediaInput = [{name: 'source', type: 'urlinput', filetype: 'media', label: 'Source'}];
        const sizeInput = !hasDimensions(editor) ? [] : [{type: 'sizeinput', name: 'dimensions', label: 'Constrain proportions', constrain: true}];
        const generalTab = {title: 'General', name: 'general', items: flatten([mediaInput, sizeInput])};
        const embedTextarea = {type: 'textarea', name: 'embed', label: 'Paste your embed code below:'};
        const embedTab = {title: 'Embed', items: [embedTextarea]};
        const advancedFormItems = [];
        if (hasAltSource(editor)) advancedFormItems.push({name: 'altsource', type: 'urlinput', filetype: 'media', label: 'Alternative source URL'});
        if (hasPoster(editor)) advancedFormItems.push({name: 'poster', type: 'urlinput', filetype: 'image', label: 'Media poster (Image URL)'});
        const advancedTab = {title: 'Advanced', name: 'advanced', items: advancedFormItems};
        const tabs = [generalTab, embedTab];
        if (advancedFormItems.length > 0) tabs.push(advancedTab);
        const body = {type: 'tabpanel', tabs};
    ...
    const get = editor => {
        const showDialog$1 = () => { showDialog(editor); };
    ...
    const isLiveEmbedNode = node => { return node.name === 'iframe' || node.name === 'video' || node.name === 'audio'; };
    const getDimension = (node, styles, dimension, defaultValue = null) => {
        if (isNonNullable(node.attr(dimension))) return node.attr(dimension);
    ...
    const setDimensions = (node, previewNode, styles) => {
        ...
        previewNode.attr({
            width: getDimension(node, styles, 'width', useDefaults ? '300' : null),
            height: getDimension(node, styles, 'height', useDefaults ? fallbackHeight : null)
        });
    };
    ...
    const createPreviewNode = (editor, node) => {
        let _a;
        const name = node.name, previewWrapper = new global$2('span', 1);
        previewWrapper.attr({
        ...
        if (name === 'iframe') previewNode.attr({allowfullscreen: node.attr('allowfullscreen'), frameborder: '0'});
        else {
            const attrs = ['controls', 'crossorigin', 'currentTime', 'loop', 'muted', 'poster', 'preload'];
            each$1(attrs, attrName => { previewNode.attr(attrName, node.attr(attrName)); });
    ...
    const isWithinEmbedWrapper = node => {
        ...
        while (tempNode = tempNode.parent) {
            if (tempNode.attr('data-ephox-embed-iri') || isPageEmbedWrapper(tempNode)) return true;
        }
        return false;
    };
    ...
    const parseAndSanitize = (editor, context, html) => {
        const getEditorOption = editor.options.get, sanitize = getEditorOption('xss_sanitization');
        const validate = shouldFilterHtml(editor);
        return Parser(editor.schema, {sanitize, validate}).parse(html, {context});
    };
    const setup$1 = editor => {
        editor.on('PreInit', () => {
            const {schema, serializer, parser} = editor, boolAttrs = schema.getBoolAttrs();
            ...
            serializer.addAttributeFilter('data-mce-object', (nodes, name) => {
                let _a, i = nodes.length;
                while (i--) {
                    const node = nodes[i];
                    if (!node.parent) continue;
                    const realElm = new global$2(node.attr(name), 1);
                    if (node.attr(name) !== 'audio' && node.attr(name) !== 'script') {
                        const className = node.attr('class');
                        if (className && className.indexOf('mce-preview-object') !== -1 && node.firstChild) realElm.attr({width: node.firstChild.attr('width'), height: node.firstChild.attr('height')});
                        else realElm.attr({width: node.attr('width'), height: node.attr('height')});
                    }
                    ...
                    while (ai--) {
                        const attrName = attribs[ai].name;
                        if (attrName.indexOf('data-mce-p-') === 0) realElm.attr(attrName.substr(11), attribs[ai].value);
                    }
                    if (node.attr(name) === 'script') realElm.attr('type', 'text/javascript');
                    const innerHtml = node.attr('data-mce-html');
                    if (innerHtml) {
                        const fragment = parseAndSanitize(editor, node.attr(name), unescape(innerHtml));
    ...
    const register = editor => {
        ...
        editor.ui.registry.addMenuItem('media', {icon: 'embed', text: 'Media...', onAction});
    };
    let Plugin = () => {
