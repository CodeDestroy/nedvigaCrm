...
(function () {
    ...
    const insertChar = (editor, chr) => { editor.execCommand('mceInsertContent', false, fireInsertCustomChar(editor, chr).chr); };
    ...
    const eq = t => a => t === a, isArray$1 = isType('array'), isNull = eq(null), isUndefined = eq(undefined);
    const isNullable = a => a === null || a === undefined, isNonNullable = a => !isNullable(a), isFunction = isSimpleType('function');
    ...
    const register$2 = editor => {
        const registerOption = editor.options.register, charMapProcessor = value => isFunction(value) || isArray$1(value);
        registerOption('charmap', {processor: charMapProcessor});
        registerOption('charmap_append', {processor: charMapProcessor});
    };
    const getCharMap$1 = option('charmap'), getCharMapAppend = option('charmap_append');
    const isArray = global.isArray, UserDefined = 'User Defined';
    ...
    const charmapFilter = charmap => { return global.grep(charmap, item => { return isArray(item) && item.length === 2; }); };
    ...
    const extendCharMap = (editor, charmap) => {
        ...
        if (userCharMap) charmap = [{name: UserDefined, characters: getCharsFromOption(userCharMap)}];
    ...
    const getCharMap = editor => {
        ...
        return groups.length > 1 ? [{name: 'All', characters: bind(groups, g => g.characters)}].concat(groups) : groups;
    };
    const get = editor => {
        const getCharMap$1 = () => { return getCharMap(editor); }, insertChar$1 = chr => { insertChar(editor, chr); };
        return {getCharMap: getCharMap$1, insertChar: insertChar$1};
    };
    const Cell = initial => {
        let value = initial;
    ...
    const scan = (group, pattern) => {
        ...
        return map(matches, m => ({text: m[1], value: fromCodePoint(m[0]), icon: fromCodePoint(m[0])}));
    };
    const patternName = 'pattern';
    const open = (editor, charMap) => {
        const makeGroupItems = () => [{label: 'Search', type: 'input', name: patternName}, {type: 'collection', name: 'results'}];
        const makeTabs = () => map(charMap, charGroup => ({title: charGroup.name, name: charGroup.name, items: makeGroupItems()}));
        ...
        const scanAndSet = (dialogApi, pattern) => {
            find(charMap, group => group.name === currentTab.get()).each(f => { dialogApi.setData({results: scan(f, pattern)}); });
        };
        const updateFilter = last(dialogApi => { scanAndSet(dialogApi, dialogApi.getData().pattern); }, 40);
        const body = charMap.length === 1 ? makePanel() : makeTabPanel();
        const initialData = {pattern: '', results: scan(charMap[0], '')};
        const bridgeSpec = {
    ...
    const register$1 = (editor, charMap) => {
        editor.addCommand('mceShowCharmap', () => { open(editor, charMap); });
    };
    ...
    const register = editor => {
        ...
        editor.ui.registry.addButton('charmap', {icon: 'insert-character', tooltip: 'Special character', onAction});
        editor.ui.registry.addMenuItem('charmap', {icon: 'insert-character', text: 'Special character...', onAction});
    };
    let Plugin = () => {
