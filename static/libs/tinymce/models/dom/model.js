...
(function () {
    'use strict';
    let global$1 = tinymce.util.Tools.resolve('tinymce.ModelManager');
    ...
    const eq$2 = t => a => t === a, isString = isType$1('string'), isObject = isType$1('object');
    const isArray = isType$1('array'), isNull = eq$2(null), isBoolean = isSimpleType('boolean');
    const isUndefined = eq$2(undefined), isNullable = a => a === null || a === undefined;
    const isNonNullable = a => !isNullable(a), isFunction = isSimpleType('function');
    const isNumber = isSimpleType('number'), noop = () => {};
    const compose = (fa, fb) => { return (...args) => { return fa(fb.apply(null, args)); }; };
    const compose1 = (fbc, fab) => a => fbc(fab(a)), constant = value => { return () => { return value; }; };
    const identity = x => { return x; }, tripleEquals = (a, b) => { return a === b; };
    function curry(fn, ...initialArgs) { return (...restArgs) => { return fn.apply(null, initialArgs.concat(restArgs)); }; }
    const not = f => t => !f(t), die = msg => { return () => { throw new Error(msg); }; };
    const apply = f => { return f(); }, never = constant(false), always = constant(true);
    class Optional {
    ...
    const nativeSlice = Array.prototype.slice, nativeIndexOf = Array.prototype.indexOf, nativePush = Array.prototype.push;
    const rawIndexOf = (ts, t) => nativeIndexOf.call(ts, t), contains$2 = (xs, x) => rawIndexOf(xs, x) > -1;
    ...
    const partition = (xs, pred) => {
        const pass = [], fail = [];
        for (let i = 0, len = xs.length; i < len; i++) {
            const arr = pred(xs[i], i) ? pass : fail;
            arr.push(xs[i]);
        }
        return {pass, fail};
    };
    ...
    const foldr = (xs, f, acc) => {
        eachr(xs, (x, i) => { acc = f(acc, x, i); });
        return acc;
    };
    const foldl = (xs, f, acc) => {
        each$2(xs, (x, i) => { acc = f(acc, x, i); });
        return acc;
    };
    ...
    const find$1 = (xs, pred) => { return findUntil(xs, pred, never); };
    const findIndex = (xs, pred) => {
        for (let i = 0, len = xs.length; i < len; i++) {
            if (pred(xs[i], i)) return Optional.some(i);
        }
        return Optional.none();
    };
    const flatten = xs => {
    ...
    const forall = (xs, pred) => {
        for (let i = 0, len = xs.length; i < len; ++i) {
            if (pred(xs[i], i) !== true) return false;
        }
        return true;
    };
    const reverse = xs => {
    ...
    const objAcc = r => (x, i) => { r[i] = x; };
    const internalFilter = (obj, pred, onTrue, onFalse) => {
        each$1(obj, (x, i) => { (pred(x, i) ? onTrue : onFalse)(x, i); });
    };
    ...
    const mapToArray = (obj, f) => {
        const r = [];
        each$1(obj, (value, name) => { r.push(f(value, name)); });
        return r;
    };
    const values = obj => { return mapToArray(obj, identity); };
    const get$c = (obj, key) => { return has$1(obj, key) ? Optional.from(obj[key]) : Optional.none(); };
    ...
    const resolve$2 = (p, scope) => { return path(p.split('.'), scope); }, unsafe = (name, scope) => { return resolve$2(name, scope); };
    const getOrDie = (name, scope) => {
        const actual = unsafe(name, scope);
        if (actual === undefined || actual === null) throw new Error(name + ' not available on this browser');
        return actual;
    };
    const getPrototypeOf = Object.getPrototypeOf, sandHTMLElement = scope => { return getOrDie('HTMLElement', scope); };
    const isPrototypeOf = x => {
        const scope = resolve$2('ownerDocument.defaultView', x);
        return isObject(x) && (sandHTMLElement(scope).prototype.isPrototypeOf(x) || /^HTML\w*Element$/.test(getPrototypeOf(x).constructor.name));
    };
    const COMMENT = 8, DOCUMENT = 9, DOCUMENT_FRAGMENT = 11, ELEMENT = 1, TEXT = 3;
    ...
    const isElement = isType(ELEMENT), isText = isType(TEXT), isDocument = isType(DOCUMENT);
    const isDocumentFragment = isType(DOCUMENT_FRAGMENT), isTag = tag => e => isElement(e) && name(e) === tag;
    ...
    const set$2 = (element, key, value) => { rawSet(element.dom, key, value); };
    const setAll$1 = (element, attrs) => { each$1(attrs, (v, k) => { rawSet(element.dom, k, v); }); };
    const setOptions = (element, attrs) => {
        each$1(attrs, (v, k) => { v.fold(() => { remove$7(element, k); }, value => { rawSet(element.dom, k, value); }); });
    };
    ...
    const remove$7 = (element, key) => { element.dom.removeAttribute(key); };
    const clone$2 = element => foldl(element.dom.attributes, (acc, attr) => {
        acc[attr.name] = attr.value;
        return acc;
    }, {});
    const fromHtml$1 = (html, scope) => {
        const doc = scope || document, div = doc.createElement('div');
        div.innerHTML = html;
    ...
    const fromTag = (tag, scope) => {
        ...
        return fromDom$1(doc.createElement(tag));
    };
    const fromText = (text, scope) => {
        const doc = scope || document;
        return fromDom$1(doc.createTextNode(text));
    };
    ...
    const SugarElement = {fromHtml: fromHtml$1, fromTag, fromText, fromDom: fromDom$1, fromPoint: fromPoint$1};
    const is$2 = (element, selector) => {
        const dom = element.dom;
        if (dom.nodeType !== ELEMENT) return false;
        else {
            if (dom.matches !== undefined) return dom.matches(selector);
            else if (dom.msMatchesSelector !== undefined) return dom.msMatchesSelector(selector);
            else if (dom.webkitMatchesSelector !== undefined) return dom.webkitMatchesSelector(selector);
            else if (dom.mozMatchesSelector !== undefined) return elem.mozMatchesSelector(selector);
    ...
    const parents = (element, isRoot) => {
        ...
        while (dom.parentNode !== null && dom.parentNode !== undefined) {
            ...
            ret.push(SugarElement.fromDom(rawParent));
            if (stop(SugarElement.fromDom(rawParent)) === true) break;
            else dom = rawParent;
        }
        return ret;
    };
    ...
    const before$3 = (marker, element) => { parent(marker).each(v => { v.dom.insertBefore(element.dom, marker.dom); }); };
    const after$5 = (marker, element) => {
        nextSibling(marker).fold(() => {
            parent(marker).each(v => { append$1(v, element); });
        }, v => {
            before$3(v, element);
        });
    };
    ...
    const appendAt = (parent, element, index) => {
        child$2(parent, index).fold(() => { append$1(parent, element); }, v => { before$3(v, element); });
    };
    ...
    const append = (parent, elements) => { each$2(elements, x => { append$1(parent, x); }); };
    const empty = element => {
        element.dom.textContent = '';
        each$2(children$2(element), rogue => { remove$6(rogue); });
    };
    const remove$6 = element => {
        if (element.dom.parentNode !== null) element.dom.parentNode.removeChild(element.dom);
    };
    const unwrap = wrapper => {
        if (children$2(wrapper).length > 0) after$4(wrapper, children);
        remove$6(wrapper);
    };
    const clone$1 = (original, isDeep) => SugarElement.fromDom(original.dom.cloneNode(isDeep));
    const shallow = original => clone$1(original, false), deep = original => clone$1(original, true);
    const shallowAs = (original, tag) => {
        setAll$1(SugarElement.fromTag(tag), clone$2(original));
        return SugarElement.fromTag(tag);
    };
    const copy$2 = (original, tag) => {
        append(shallowAs(original, tag), children$2(deep(original)));
        return shallowAs(original, tag);
    };
    const mutate$1 = (original, tag) => {
        after$5(original, shallowAs(original, tag));
        append(shallowAs(original, tag), children$2(original));
        remove$6(original);
        return shallowAs(original, tag);
    };
    const validSectionList = ['tfoot', 'thead', 'tbody', 'colgroup'];
    const isValidSection = parentName => contains$2(validSectionList, parentName);
    const grid = (rows, columns) => ({rows, columns});
    const address = (row, column) => ({row, column});
    const detail = (element, rowspan, colspan) => ({element, rowspan, colspan});
    const detailnew = (element, rowspan, colspan, isNew) => ({element, rowspan, colspan, isNew});
    const extended = (element, rowspan, colspan, row, column, isLocked) => ({element, rowspan, colspan, row, column, isLocked});
    const rowdetail = (element, cells, section) => ({element, cells, section});
    const rowdetailnew = (element, cells, section, isNew) => ({element, cells, section, isNew});
    const elementnew = (element, isNew, isLocked) => ({element, isNew, isLocked});
    const rowcells = (element, cells, section, isNew) => ({element, cells, section, isNew});
    const bounds = (startRow, startCol, finishRow, finishCol) => ({startRow, startCol, finishRow, finishCol});
    const columnext = (element, colspan, column) => ({element, colspan, column});
    const colgroup = (element, columns) => ({element, columns});
    const isShadowRoot = dos => isDocumentFragment(dos) && isNonNullable(dos.dom.host);
    const supported = isFunction(Element.prototype.attachShadow) && isFunction(Node.prototype.getRootNode);
    const isSupported$1 = constant(supported);
    const getRootNode = supported ? e => SugarElement.fromDom(e.dom.getRootNode()) : documentOrOwner;
    const getShadowRoot = e => { return isShadowRoot(getRootNode(e)) ? Optional.some(getRootNode(e)) : Optional.none(); };
        ...
        if (isSupported$1() && isNonNullable(event.target)) {
            ...
            if (isElement(el) && isOpenShadowHost(el)) {
                if (event.composed && event.composedPath) {
                    if (event.composedPath()) return head(event.composedPath());
                }
            }
        }
    ...
    const inBody = element => {
        ...
        return getShadowRoot(SugarElement.fromDom(dom)).fold(() => dom.ownerDocument.body.contains(dom), compose1(inBody, getShadowHost));
    };
    const body$1 = () => getBody$1(SugarElement.fromDom(document));
    const getBody$1 = doc => {
        if (doc.dom.body === null || doc.dom.body === undefined) throw new Error('Body is not available yet');
        return SugarElement.fromDom(doc.dom.body);
    };
    ...
    const closest$2 = (scope, predicate, isRoot) => { return ClosestOrAncestor((s, test) => test(s), ancestor$2, scope, predicate, isRoot); };
    const child$1 = (scope, predicate) => { return find$1(scope.dom.childNodes, node => predicate(SugarElement.fromDom(node))).map(SugarElement.fromDom); };
    const descendant$1 = (scope, predicate) => {
        const descend = node => {
            for (let i = 0; i < node.childNodes.length; i++) {
                if (predicate(SugarElement.fromDom(node.childNodes[i]))) return Optional.some(SugarElement.fromDom(node.childNodes[i]));
                if (descend(node.childNodes[i]).isSome()) return descend(node.childNodes[i]);
            }
            return Optional.none();
        };
    ...
    const closest$1 = (scope, selector, isRoot) => {
        return ClosestOrAncestor((element, selector) => is$2(element, selector), ancestor$1, scope, selector, isRoot);
    };
    const is = (lhs, rhs, comparator = tripleEquals) => lhs.exists(left => comparator(left, rhs));
    const cat = arr => {
        const r = [];
        const push = x => { r.push(x); };
        for (let i = 0; i < arr.length; i++) {
            arr[i].each(push);
        }
        return r;
    ...
    const contains = (str, substr, start = 0, end) => {
        if (str.indexOf(substr, start) !== -1) return isUndefined(end) ? true : str.indexOf(substr, start) + substr.length <= end;
        return false;
    };
    const startsWith = (str, prefix) => { return checkRange(str, prefix, 0); };
    const endsWith = (str, suffix) => { return checkRange(str, suffix, str.length - suffix.length); };
    const blank = r => s => s.replace(r, ''), trim = blank(/^\s+|\s+$/g), isNotEmpty = s => s.length > 0;
    const toFloat = value => { return isNaN(parseFloat(value)) ? Optional.none() : Optional.some(parseFloat(value)); };
    ...
    const set$1 = (element, property, value) => { internalSet(element.dom, property, value); };
    const setAll = (element, css) => { each$1(css, (v, k) => { internalSet(element.dom, k, v); }); };
    ...
    const getRaw$2 = (element, property) => { return Optional.from(getUnsafeProperty(element.dom, property)).filter(r => r.length > 0); };
    ...
    const copy$1 = (source, target) => {
        if (isSupported(source.dom) && isSupported(target.dom)) target.dom.style.cssText = source.dom.style.cssText;
    };
    ...
    const minWidth = constant(10), minHeight = constant(10);
    const firstLayer = (scope, selector) => { return filterFirstLayer(scope, selector, always); };
    ...
    const lookup = (tags, element, isRoot = never) => {
        ...
        return ancestor$1(element, tags.join(','), elm => is$2(elm, 'table') || isRoot(elm));
    };
    ...
    const columns$1 = ancestor => {
        if (is$2(ancestor, 'colgroup')) return children(ancestor, 'col');
    ...
    const fromRowsOrColGroups = (elems, getSection) => map$1(elems, row => {
        if (name(row) === 'colgroup') return rowdetail(row, map$1(columns$1(row), column => { return detail(column, 1, getAttrValue(column, 'span', 1)); }), 'colgroup');
        return rowdetail(row, map$1(cells$1(row), cell => { return detail(cell, getAttrValue(cell, 'rowspan', 1), getAttrValue(cell, 'colspan', 1)); }), getSection(row));
    });
    const getParentSection = group => parent(group).map(parent => { return isValidSection(name(parent)) ? name(parent) : 'tbody'; }).getOr('tbody');
    const fromTable$1 = table => { return fromRowsOrColGroups([...columnGroups(table), ...rows$1(table)], getParentSection); };
    ...
    const DeviceType = (os, browser, userAgent, mediaMatch) => {
        const isiPad = os.isiOS() && /ipad/i.test(userAgent) === true, isiPhone = os.isiOS() && !isiPad;
        const isMobile = os.isiOS() || os.isAndroid(), isTouch = isMobile || mediaMatch('(pointer:coarse)');
    ...
    const firstMatch = (regexes, s) => {
        for (let i = 0; i < regexes.length; i++) {
            if (regexes[i].test(s)) return regexes[i];
        }
        return undefined;
    };
    const find = (regexes, agent) => {
        if (!firstMatch(regexes, agent)) return {major: 0, minor: 0};
        const group = i => { return Number(agent.replace(firstMatch(regexes, agent), '$' + i)); };
        return nu$2(group(1), group(2));
    };
    const detect$5 = (versionRegexes, agent) => {
        if (versionRegexes.length === 0) return unknown$2();
        return find(versionRegexes, String(agent).toLowerCase());
    };
    const unknown$2 = () => { return nu$2(0, 0); };
    const nu$2 = (major, minor) => { return {major, minor}; };
    const Version = {nu: nu$2, detect: detect$5, unknown: unknown$2};
    const detectBrowser$1 = (browsers, userAgentData) => {
        return findMap(userAgentData.brands, uaBrand => {
            ...
            }).map(info => ({current: info.name, version: Version.nu(parseInt(uaBrand.version, 10), 0)}));
        });
    };
    const detect$4 = (candidates, userAgent) => {
        return find$1(candidates, candidate => { return candidate.search(String(userAgent).toLowerCase()); });
    };
    const detectBrowser = (browsers, userAgent) => {
        return detect$4(browsers, userAgent).map(browser => {
            const version = Version.detect(browser.versionRegexes, userAgent);
            return {current: browser.name, version};
        });
    };
    const detectOs = (oses, userAgent) => {
        return detect$4(oses, userAgent).map(os => {
            const version = Version.detect(os.versionRegexes, userAgent);
            return {current: os.name, version};
        });
    };
    const normalVersionRegex = /.*?version\/\ ?([0-9]+)\.([0-9]+).*/;
    const checkContains = target => { return uastring => { return contains(uastring, target); }; };
    const browsers = [{
        name: 'Edge',
        ...
        search: uastring => { return contains(uastring, 'chrome') && !contains(uastring, 'chromeframe'); }
    }, {
        name: 'IE',
        versionRegexes: [/.*?msie\ ?([0-9]+)\.([0-9]+).*/, /.*?rv:([0-9]+)\.([0-9]+).*/],
        search: uastring => { return contains(uastring, 'msie') || contains(uastring, 'trident'); }
    }, {
        name: 'Opera',
        ...
        search: uastring => { return (contains(uastring, 'safari') || contains(uastring, 'mobile/')) && contains(uastring, 'applewebkit'); }
    }];
    const oses = [{
        name: 'Windows',
        ...
        name: 'iOS',
        search: uastring => { return contains(uastring, 'iphone') || contains(uastring, 'ipad'); },
    ...
    const PlatformInfo = {browsers: constant(browsers), oses: constant(oses)};
    const edge = 'Edge', chromium = 'Chromium', ie = 'IE', opera = 'Opera';
    const firefox = 'Firefox', safari = 'Safari';
    const unknown$1 = () => { return nu$1({current: undefined, version: Version.unknown()});};
    const nu$1 = info => {
        const current = info.current, version = info.version, isBrowser = name => () => current === name;
        return {current, version, isEdge: isBrowser(edge), isChromium: isBrowser(chromium), isIE: isBrowser(ie), isOpera: isBrowser(opera), isFirefox: isBrowser(firefox), isSafari: isBrowser(safari)};
    };
    const Browser = {unknown: unknown$1, nu: nu$1, edge: constant(edge), chromium: constant(chromium), ie: constant(ie), opera: constant(opera), firefox: constant(firefox), safari: constant(safari)};
    const windows = 'Windows', ios = 'iOS', android = 'Android', linux = 'Linux', macos = 'macOS';
    const solaris = 'Solaris', freebsd = 'FreeBSD', chromeos = 'ChromeOS';
    const unknown = () => { return nu({current: undefined, version: Version.unknown()}); };
    const nu = info => {
        const current = info.current, version = info.version, isOS = name => () => current === name;
        return {current, version, isWindows: isOS(windows), isiOS: isOS(ios), isAndroid: isOS(android), isMacOS: isOS(macos), isLinux: isOS(linux), isSolaris: isOS(solaris), isFreeBSD: isOS(freebsd), isChromeOS: isOS(chromeos)};
    };
    const OperatingSystem = {unknown, nu, windows: constant(windows), ios: constant(ios), android: constant(android), linux: constant(linux), macos: constant(macos), solaris: constant(solaris), freebsd: constant(freebsd), chromeos: constant(chromeos)};
    const detect$3 = (userAgent, userAgentDataOpt, mediaMatch) => {
        const browsers = PlatformInfo.browsers(), oses = PlatformInfo.oses();
    ...
    const Dimension = (name, getOffset) => {
        const set = (element, h) => {
            ...
            if (isSupported(element.dom)) element.dom.style[name] = h + 'px';
        };
        const get = element => {
            if (getOffset(element) <= 0 || getOffset(element) === null) return parseFloat(get$a(element, name)) || 0;
            return getOffset(element);
        };
        const getOuter = get;
        const aggregate = (element, properties) => foldl(properties, (acc, property) => {
            const value = get$a(element, property) === undefined ? 0 : parseInt(get$a(element, property), 10);
            return isNaN(value) ? acc : acc + value;
        }, 0);
        const max = (element, value, properties) => { return value > aggregate(element, properties) ? value - aggregate(element, properties) : 0; };
    ...
    const getCalculatedWidth = (element, boxSizing) => {
        const width = element.dom.getBoundingClientRect().width || element.dom.offsetWidth;
    ...
    const get$9 = element => api$2.get(element), getOuter$2 = element => api$2.getOuter(element);
    const getInner = getInnerWidth, getRuntime$1 = getWidth;
    const addCells = (gridRow, index, cells) => { return setCells(gridRow, gridRow.cells.slice(0, index).concat(cells).concat(gridRow.cells.slice(index))); };
    const addCell = (gridRow, index, cell) => addCells(gridRow, index, [cell]);
    const mutateCell = (gridRow, index, cell) => {
        gridRow.cells[index] = cell;
    };
    const setCells = (gridRow, cells) => rowcells(gridRow.element, cells, gridRow.section, gridRow.isNew);
    const mapCells = (gridRow, f) => { return rowcells(gridRow.element, map$1(gridRow.cells, f), gridRow.section, gridRow.isNew); };
    const getCell = (gridRow, index) => gridRow.cells[index];
    const getCellElement = (gridRow, index) => getCell(gridRow, index).element;
    const cellLength = gridRow => gridRow.cells.length;
    const extractGridDetails = grid => {
        const result = partition(grid, row => row.section === 'colgroup');
        return {rows: result.fail, cols: result.pass};
    };
    const clone = (gridRow, cloneRow, cloneCell) => { return rowcells(cloneRow(gridRow.element), map$1(gridRow.cells, cloneCell), gridRow.section, true); };
    ...
    const getLockedColumnsFromGrid = grid => {
        ...
        return sort$1(mapToArray(locked, (_val, key) => parseInt(key, 10)));
    };
    const key = (row, column) => { return row + ',' + column; };
    const getAt = (warehouse, row, column) => Optional.from(warehouse.access[key(row, column)]);
    const findItem = (warehouse, item, comparator) => {
        const filtered = filterItems(warehouse, detail => { return comparator(item, detail.element); });
        return filtered.length > 0 ? Optional.some(filtered[0]) : Optional.none();
    };
    const filterItems = (warehouse, predicate) => {
        const all = bind$2(warehouse.all, r => { return r.cells; });
    ...
    const generateColumns = rowData => {
        ...
        each$2(rowData.cells, column => {
            ...
            range$1(colspan, columnIndex => { columnsGroup[index + columnIndex] = columnext(column.element, colspan, index + columnIndex); });
            index += colspan;
        });
        return columnsGroup;
    };
    const generate$1 = list => {
        ...
        let maxRows = 0, maxColumns = 0, rowCount = 0;
            ...
            each$2(rowData.cells, rowCell => {
                ...
                for (let occupiedColumnPosition = 0; occupiedColumnPosition < rowCell.colspan; occupiedColumnPosition++) {
                    for (let occupiedRowPosition = 0; occupiedRowPosition < rowCell.rowspan; occupiedRowPosition++) {
                        const rowPosition = rowCount + occupiedRowPosition, columnPosition = start + occupiedColumnPosition;
        ...
        const {columns, colgroups} = last$2(colgroupRows).map(rowData => {
            ...
            return {colgroups: [colgroup$1], columns};
        }).getOrThunk(() => ({colgroups: [], columns: {}}));
        const grid$1 = grid(maxRows, maxColumns);
        return {grid: grid$1, access, all: cells, columns, colgroups};
    };
    const fromTable = table => { return generate$1(fromTable$1(table)); };
    const justCells = warehouse => bind$2(warehouse.all, w => w.cells);
    const justColumns = warehouse => values(warehouse.columns);
    const hasColumns = warehouse => keys(warehouse.columns).length > 0;
    const getColumnAt = (warehouse, columnIndex) => Optional.from(warehouse.columns[columnIndex]);
    const Warehouse = {fromTable, generate: generate$1, getAt, findItem, filterItems, justCells, justColumns, hasColumns, getColumnAt};
    const columns = (warehouse, isValidCell = always) => {
        const grid = warehouse.grid, cols = range$1(grid.columns, identity), rowsArr = range$1(grid.rows, identity);
    ...
    const decide = (getBlock, isValid, getFallback) => {
        const inBlock = getBlock(), validInBlock = find$1(inBlock, isValid);
        const detailOption = validInBlock.orThunk(() => Optional.from(inBlock[0]).orThunk(getFallback));
        return detailOption.map(detail => detail.element);
    };
    const rows = warehouse => {
        const grid = warehouse.grid, rowsArr = range$1(grid.rows, identity), cols = range$1(grid.columns, identity);
    ...
    const deduce = (xs, index) => {
        ...
        const current = xs[index].fold(() => {
            return findMap(reverse(xs.slice(0, index)), (a, i) => a.map(aa => ({value: aa, delta: i + 1})));
        }, c => Optional.some({value: c, delta: 0}));
        const next = xs[index + 1].fold(() => {
            return findMap(xs.slice(index + 1), (a, i) => a.map(aa => ({value: aa, delta: i + 1})));
        }, n => Optional.some({value: n, delta: 1}));
        return current.bind(c => next.map(n => { return Math.abs(n.value - c.value) / (n.delta + c.delta); }));
    };
    const onDirection = (isLtr, isRtl) => element => getDirection(element) === 'rtl' ? isRtl : isLtr;
    const getDirection = element => get$a(element, 'direction') === 'rtl' ? 'rtl' : 'ltr';
    const api$1 = Dimension('height', element => { return inBody(element) ? element.dom.getBoundingClientRect().height : element.dom.offsetHeight; });
    const get$8 = element => api$1.get(element), getOuter$1 = element => api$1.getOuter(element), getRuntime = getHeight$1;
    const SugarPosition = (left, top) => {
        const translate = (x, y) => SugarPosition(left + x, top + y);
    ...
    const absolute = element => {
        const doc = element.dom.ownerDocument, body = doc.body, win = doc.defaultView;
    ...
    const viewport = element => {
        const dom = element.dom, doc = dom.ownerDocument, body = doc.body;
        if (body === dom) return SugarPosition(body.offsetLeft, body.offsetTop);
        if (!inBody(element)) return SugarPosition(0, 0);
        return boxPosition(dom);
    };
    const rowInfo = (row, y) => ({row, y}), colInfo = (col, x) => ({col, x});
    const rtlEdge = cell => { return absolute(cell).left + getOuter$2(cell); }, ltrEdge = cell => { return absolute(cell).left; };
    const getLeftEdge = (index, cell) => { return colInfo(index, ltrEdge(cell)); };
    const getRightEdge = (index, cell) => { return colInfo(index, rtlEdge(cell)); };
    const getTop$1 = cell => { return absolute(cell).top; };
    const getTopEdge = (index, cell) => { return rowInfo(index, getTop$1(cell)); };
    const getBottomEdge = (index, cell) => { return rowInfo(index, getTop$1(cell) + getOuter$1(cell)); };
    const findPositions = (getInnerEdge, getOuterEdge, array) => {
        if (array.length === 0) return [];
        const lines = map$1(array.slice(1), (cellOption, index) => { return cellOption.map(cell => { return getInnerEdge(index, cell); }); });
        const lastLine = array[array.length - 1].map(cell => { return getOuterEdge(array.length - 1, cell); });
        return lines.concat([lastLine]);
    };
    const negate = step => { return -step; };
    const height = {delta: identity, positions: optElements => findPositions(getTopEdge, getBottomEdge, optElements), edge: getTop$1};
    const ltr$1 = {delta: identity, edge: ltrEdge, positions: optElements => findPositions(getLeftEdge, getRightEdge, optElements)};
    const rtl$1 = {delta: negate, edge: rtlEdge, positions: optElements => findPositions(getRightEdge, getLeftEdge, optElements)};
    const detect$1 = onDirection(ltr$1, rtl$1);
    const width = {delta: (amount, table) => detect$1(table).delta(amount, table), positions: (cols, table) => detect$1(table).positions(cols, table), edge: cell => detect$1(cell).edge(cell)};
    const units = {
    ...
    const pattern = (() => {
        const decimalDigits = '[0-9]+', signedInteger = '[+-]?' + decimalDigits, exponentPart = '[eE]' + signedInteger;
        const dot = '\\.', opt = input => `(?:${input})?`;
    ...
    const parse = (input, accepted) => {
        ...
        return match.bind(array => {
            ...
            if (isUnit(unitRaw, accepted)) return Optional.some({value, unit: unitRaw});
            return Optional.none();
        });
    };
    const rPercentageBasedSizeRegex = /(\d+(\.\d+)?)%/, rPixelBasedSizeRegex = /(\d+(\.\d+)?)px|em/;
    const isCol$2 = isTag('col');
    const getPercentSize = (elm, outerGetter, innerGetter) => {
        const relativeParent = parentElement(elm).getOrThunk(() => getBody$1(owner(elm)));
        return outerGetter(elm) / innerGetter(relativeParent) * 100;
    };
    const setPixelWidth = (cell, amount) => { set$1(cell, 'width', amount + 'px'); };
    const setPercentageWidth = (cell, amount) => { set$1(cell, 'width', amount + '%'); };
    const setHeight = (cell, amount) => { set$1(cell, 'height', amount + 'px'); };
    const getHeightValue = cell => getRuntime(cell) + 'px';
    const convert = (cell, number, getter, setter) => {
        const newSize = table(cell).map(table => { return Math.floor(number / 100 * getter(table)); }).getOr(number);
        setter(cell, newSize);
        return newSize;
    };
    const normalizePixelSize = (value, cell, getter, setter) => { return endsWith(value, '%') && name(cell) !== 'table' ? convert(cell, parseFloat(value), getter, setter) : parseFloat(value); };
    const getTotalHeight = cell => {
        const value = getHeightValue(cell);
        if (!value) return get$8(cell);
        return normalizePixelSize(value, cell, get$8, setHeight);
    };
    const get$7 = (cell, type, f) => { return f(cell) / getSpan(cell, type); };
    const getRaw$1 = (element, prop) => {
        return getRaw$2(element, prop).orThunk(() => { return getOpt(element, prop).map(val => val + 'px'); });
    };
    ...
    const getHeight = cell => { return get$7(cell, 'rowspan', getTotalHeight); };
    const getGenericWidth = cell => {
        return getRawWidth$1(cell).bind(w => parse(w, ['fixed', 'relative', 'empty']));
    };
    const setGenericWidth = (cell, amount, unit) => { set$1(cell, 'width', amount + unit); };
    const getPixelTableWidth = table => get$9(table) + 'px', getPercentTableWidth = table => getPercentSize(table, get$9, getInner) + '%';
    ...
    const isNoneSizing$1 = table => getRawWidth$1(table).isNone(), percentageBasedSizeRegex = constant(rPercentageBasedSizeRegex);
    const isCol$1 = isTag('col'), getRawW = cell => { return getRawWidth$1(cell).getOrThunk(() => getPixelWidth$1(cell) + 'px'); };
    const getRawH = cell => { return getRawHeight(cell).getOrThunk(() => getHeight(cell) + 'px'); };
    ...
    const getWidthFrom = (warehouse, table, getWidth, fallback) => {
        ...
        return map$1(columns$1, (cellOption, c) => {
            return getDimension(cellOption, c, backups, colFilter, column => {
                ...
                return getDimension(bindFrom(columnCells[c], identity), c, backups, colFilter, cell => fallback(Optional.some(get$9(cell))), fallback);
            }, fallback);
        });
    };
    const getDeduced = deduced => { return deduced.map(d => { return d + 'px'; }).getOr(''); };
    const getRawWidths = (warehouse, table) => { return getWidthFrom(warehouse, table, getRawW, getDeduced); };
    const getPercentageWidths = (warehouse, table, tableSize) => {
        return getWidthFrom(warehouse, table, getPercentageWidth, deduced => {
            return deduced.fold(() => { return tableSize.minCellWidth(); }, cellWidth => { return cellWidth / tableSize.pixelWidth() * 100; });
        });
    };
    const getPixelWidths = (warehouse, table, tableSize) => {
        return getWidthFrom(warehouse, table, getPixelWidth$1, deduced => { return deduced.getOrThunk(tableSize.minCellWidth); });
    };
    ...
    const getPixelHeights = (warehouse, table, direction) => {
        return getHeightFrom(warehouse, table, direction, getHeight, deduced => { return deduced.getOrThunk(minHeight); });
    };
    const getRawHeights = (warehouse, table, direction) => { return getHeightFrom(warehouse, table, direction, getRawH, getDeduced); };
    const widthLookup = (table, getter) => () => {
        if (inBody(table)) return getter(table);
        return parseFloat(getRaw$2(table, 'width').getOr('0'));
    };
    const noneSize = table => {
        const getWidth = widthLookup(table, get$9), zero = constant(0);
    ...
    const percentageSize = table => {
        ...
        const getWidth = widthLookup(table, get$9), getCellDelta = delta => delta / getWidth() * 100;
        const singleColumnWidth = (w, _delta) => [100 - w], minCellWidth = () => minWidth() / getWidth() * 100;
        const adjustTableWidth = delta => {
            const currentWidth = getFloatWidth(), change = delta / 100 * currentWidth;
    ...
    const pixelSize = table => {
        const getWidth = widthLookup(table, get$9), getCellDelta = identity;
        const singleColumnWidth = (w, delta) => { return [Math.max(minWidth(), w + delta) - w]; };
        const adjustTableWidth = delta => { setPixelWidth(table, getWidth() + delta); };
        const getWidths = (warehouse, tableSize) => getPixelWidths(warehouse, table, tableSize);
        return {
            width: getWidth,
            pixelWidth: getWidth,
    ...
    const getTableSize = table => { return getRawWidth$1(table).fold(() => noneSize(table), w => chooseSize(table, w)); };
    const TableSize = {getTableSize, pixelSize, percentageSize, noneSize};
    const statsStruct = (minRow, minCol, maxRow, maxCol, allCells, selectedCells) => ({minRow, minCol, maxRow, maxCol, allCells, selectedCells});
    const findSelectedStats = (house, isSelected) => {
        let minRow = house.grid.rows, minCol = house.grid.columns, maxRow = 0, maxCol = 0;
        const allCells = [], selectedCells = [];
        each$1(house.access, detail => {
            allCells.push(detail);
            if (isSelected(detail)) {
                selectedCells.push(detail);
                const startRow = detail.row, endRow = startRow + detail.rowspan - 1;
                const startCol = detail.column, endCol = startCol + detail.colspan - 1;
    ...
    const makeCell = (list, seenSelected, rowIndex) => {
        ...
        f(list[rowIndex].element, td);
    };
    const fillInGaps = (list, house, stats, isSelected) => {
        const rows = filter$2(list, row => row.section !== 'colgroup'), totalColumns = house.grid.columns;
    ...
    const extract$1 = (table, selectedSelector) => {
        const isSelected = detail => is$2(detail.element, selectedSelector), replica = deep(table);
        const list = fromTable$1(replica), tableSize = TableSize.getTableSize(table);
        const replicaHouse = Warehouse.generate(list), replicaStats = findSelectedStats(replicaHouse, isSelected);
        ...
        const house = Warehouse.fromTable(table), widthDelta = getTableWidthDelta(table, house, tableSize, replicaStats);
    ...
    const api = NodeValue(isText, 'text'), get$6 = element => api.get(element);
    const getOption = element => api.getOption(element), set = (element, value) => api.set(element, value);
    ...
    const first = element => descendant$1(element, isCursorPosition), last$1 = element => descendantRtl(element, isCursorPosition);
    ...
    const createCol = doc => () => { return SugarElement.fromTag('col', doc.dom); };
    const createColgroup = doc => () => { return SugarElement.fromTag('colgroup', doc.dom); };
    const createRow$1 = doc => () => { return SugarElement.fromTag('tr', doc.dom); };
    ...
    const pasteReplace = cell => { return cell; };
    const cloneFormats = (oldCell, newCell, formats) => {
        const first$1 = first(oldCell);
        return first$1.map(firstText => {
            const formatSelector = formats.join(',');
            const parents = ancestors$3(firstText, formatSelector, element => { return eq$1(element, oldCell); });
    ...
    const cellOperations = (mutate, doc, formatsToClone) => {
        ...
        return {col: newCol, colgroup: createColgroup(doc), row: createRow$1(doc), cell: newCell, replace: replace$1, colGap: createCol(doc), gap: createCell(doc)};
    };
    const paste$1 = doc => {
        return {col: createCol(doc), colgroup: createColgroup(doc), row: createRow$1(doc), cell: createCell(doc), replace: pasteReplace, colGap: createCol(doc), gap: createCell(doc)};
    };
    ...
    const getRaw = element => element.dom.contentEditable, getBody = editor => SugarElement.fromDom(editor.getBody());
    ...
    const getPixelWidth = elm => elm.getBoundingClientRect().width, getPixelHeight = elm => elm.getBoundingClientRect().height;
    const getRawWidth = (editor, elm) => {
        const raw = editor.dom.getStyle(elm, 'width') || editor.dom.getAttrib(elm, 'width');
        return Optional.from(raw).filter(isNotEmpty);
    };
    const isPercentage$1 = value => /^(\d+(\.\d+)?)%$/.test(value), isPixel = value => /^(\d+(\.\d+)?)px$/.test(value);
    const isInEditableContext$1 = cell => closest$2(cell, isTag('table')).exists(isEditable$1);
    const inSelection = (bounds, detail) => {
        const leftEdge = detail.column, rightEdge = detail.column + detail.colspan - 1;
        const topEdge = detail.row, bottomEdge = detail.row + detail.rowspan - 1;
    ...
    const getAnyBox = (warehouse, startCell, finishCell) => {
        ...
        return startCoords.bind(sc => { return finishCoords.map(fc => { return getBounds(sc, fc); }); });
    };
    const getBox$1 = (warehouse, startCell, finishCell) => {
        return getAnyBox(warehouse, startCell, finishCell).bind(bounds => { return isRectangular(warehouse, bounds); });
    };
    ...
    const intercepts$1 = (warehouse, start, finish) => {
        return getAnyBox(warehouse, start, finish).map(bounds => {
            return map$1(Warehouse.filterItems(warehouse, curry(inSelection, bounds)), detail => { return detail.element; });
        });
    };
    const parentCell = (warehouse, innerCell) => {
        const isContainedBy = (c1, c2) => { return contains$1(c2, c1); };
        return Warehouse.findItem(warehouse, innerCell, isContainedBy).map(detail => { return detail.element; });
    };
    const moveBy = (cell, deltaRow, deltaColumn) => {
        return table(cell).bind(table => { return moveBy$1(getWarehouse(table), cell, deltaRow, deltaColumn); });
    };
    const intercepts = (table, first, last) => {
        return intercepts$1(getWarehouse(table), first, last);
    };
    ...
    const getBox = (table, first, last) => { return getBox$1(getWarehouse(table), first, last); };
    const getWarehouse = Warehouse.fromTable;
    let TagBoundaries = ['body', 'p', 'div', 'article', 'aside', 'figcaption', 'figure', 'footer', 'header', 'nav', 'section', 'ol', 'ul', 'li', 'table', 'thead', 'tbody', 'tfoot', 'caption', 'tr', 'td', 'th', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'pre', 'address'];
    let DomUniverse = () => {
        const clone = element => { return SugarElement.fromDom(element.dom.cloneNode(false)); };
        ...
        const comparePosition = (element, other) => { return element.dom.compareDocumentPosition(other.dom); };
        const copyAttributesTo = (source, destination) => { setAll$1(destination, clone$2(source)); };
        const isSpecial = element => { return contains$2(['script', 'noscript', 'iframe', 'noframes', 'noembed', 'title', 'style', 'textarea', 'xmp'], name(element)); };
        const getLanguage = element => isElement(element) ? getOpt(element, 'lang') : Optional.none();
        return {
            up: constant({selector: ancestor$1, closest: closest$1, predicate: ancestor$2, all: parents}),
            down: constant({selector: descendants, predicate: descendants$1}),
            styles: constant({get: get$a, getRaw: getRaw$2, set: set$1, remove: remove$5}),
            attrs: constant({get: get$b, set: set$2, remove: remove$7, copyTo: copyAttributesTo}),
            insert: constant({
                before: before$3,
                after: after$5,
                afterAll: after$4,
            ...
            remove: constant({unwrap: unwrap, remove: remove$6}),
            create: constant({nu: SugarElement.fromTag, clone, text: SugarElement.fromText}),
            query: constant({comparePosition, prevSibling: prevSibling, nextSibling: nextSibling}),
            property: constant({
                children: children$2,
                name: name,
                parent: parent,
    ...
    const all = (universe, look, elements, f) => { return f(universe, look, elements[0], elements.slice(1)); };
    const oneAll = (universe, look, elements) => { return elements.length > 0 ? all(universe, look, elements, unsafeOne) : Optional.none(); };
    const unsafeOne = (universe, look, head, tail) => {
        return foldr(tail, (b, a) => { return commonElement(universe, b, look(universe, a)); }, look(universe, head));
    };
    const commonElement = (universe, start, end) => {
        return start.bind(s => { return end.filter(curry(universe.eq, s)); });
    };
    const eq = (universe, item) => { return curry(universe.eq, item); };
    const ancestors$2 = (universe, start, end, isRoot = never) => {
        const prune = path => {
            return findIndex(path, isRoot).fold(() => { return path; }, ind => { return path.slice(0, ind + 1); });
        };
        const pruned1 = prune([start].concat(universe.up().all(start)));
        const pruned2 = prune([end].concat(universe.up().all(end)));
        const shared = find$1(pruned1, x => { return exists(pruned2, eq(universe, x)); });
        return {firstpath: pruned1, secondpath: pruned2, shared};
    };
    const sharedOne$1 = oneAll, ancestors$1 = ancestors$2, universe$3 = DomUniverse();
    const sharedOne = (look, elements) => {
        return sharedOne$1(universe$3, (_universe, element) => { return look(element); }, elements);
    };
    const ancestors = (start, finish, isRoot) => { return ancestors$1(universe$3, start, finish, isRoot); };
    const lookupTable = container => { return ancestor$1(container, 'table'); };
    const identify = (start, finish, isRoot) => {
        const getIsRoot = rootTable => {
            return element => { return isRoot !== undefined && isRoot(element) || eq$1(element, rootTable); };
        };
        if (eq$1(start, finish)) return Optional.some({boxes: Optional.some([start]), start, finish});
        else {
            return lookupTable(start).bind(startTable => {
                return lookupTable(finish).bind(finishTable => {
                    if (eq$1(startTable, finishTable)) return Optional.some({boxes: intercepts(startTable, start, finish), start, finish});
                        ...
                        return Optional.some({
                            ...
                            start, finish: ancestorCells.length > 0 ? ancestorCells[ancestorCells.length - 1] : finish
                        });
                    } else if (contains$1(finishTable, startTable)) {
                        const ancestorCells = ancestors$3(start, 'td,th', getIsRoot(finishTable));
                        return Optional.some({
                            boxes: nestedIntercepts(finishTable, start, startTable, finish, finishTable),
                            start, finish: ancestorCells.length > 0 ? ancestorCells[ancestorCells.length - 1] : start
                        });
                    } else {
                        return ancestors(start, finish).shared.bind(lca => {
                            return closest$1(lca, 'table', isRoot).bind(lcaTable => {
                                ...
                                return Optional.some({
                                    ...
                                    start: startAncestorCells.length > 0 ? startAncestorCells[startAncestorCells.length - 1] : start,
                                    finish: finishAncestorCells.length > 0 ? finishAncestorCells[finishAncestorCells.length - 1] : finish
                                });
                            });
                        });
                    }
            ...
            return descendant(table, firstSelectedSelector).bind(start => {
                return identify(start, finish).bind(identified => {
                    return identified.boxes.map(boxes => {
                        return {boxes, start: identified.start, finish: identified.finish};
                    });
                });
            });
        });
    ...
    const shiftSelection = (boxes, deltaRow, deltaColumn, firstSelectedSelector, lastSelectedSelector) => {
        return getLast(boxes, lastSelectedSelector).bind(last => {
            return moveBy(last, deltaRow, deltaColumn).bind(finish => {return expandTo(finish, firstSelectedSelector);});
        });
    };
    const retrieve = (container, selector) => { return retrieve$1(container, selector); };
    const retrieveBox = (container, firstSelectedSelector, lastSelectedSelector) => {
        return getEdges(container, firstSelectedSelector, lastSelectedSelector).bind(edges => {
            const isRoot = ancestor => { return eq$1(container, ancestor); };
            const sectionSelector = 'thead,tfoot,tbody,table';
            const firstAncestor = ancestor$1(edges.first, sectionSelector, isRoot);
            const lastAncestor = ancestor$1(edges.last, sectionSelector, isRoot);
            return firstAncestor.bind(fA => {
                return lastAncestor.bind(lA => { return eq$1(fA, lA) ? getBox(edges.table, edges.first, edges.last) : Optional.none(); });
            });
        });
    };
    ...
    const mergable = (table, selectedCells, ephemera) => {
        ...
        return retrieveBox(table, ephemera.firstSelectedSelector, ephemera.lastSelectedSelector).map(bounds => ({bounds, cells: selectedCells}));
    };
    ...
    const paste = (element, clipboard, generators) => ({element, clipboard, generators});
    const pasteRows = (selectedCells, _cell, clipboard, generators) => ({selection: selection(selectedCells), clipboard, generators});
            ...
            if (e.selection === true && e.paste === true) {
                ...
                head(selectedCells).each(cell => {
                    table(cell).each(table => {
                        const elements = filter$2(fromHtml(e.content), content => { return name(content) !== 'meta'; });
                        const isTable = isTag('table');
                        if (elements.length === 1 && isTable(elements[0])) {
                            e.preventDefault();
                            const doc = SugarElement.fromDom(editor.getDoc());
                            const generators = paste$1(doc), targets = paste(cell, elements[0], generators);
                            actions.pasteCells(table, targets).each(() => { editor.focus(); });
                        }
                    });
                });
            }
    ...
    const point = (element, offset) => ({element, offset});
    const scan$1 = (universe, element, direction) => {
        if (universe.property().isText(element) && universe.property().getText(element).trim().length === 0 || universe.property().isComment(element)) {
            return direction(element).bind(elem => {
                return scan$1(universe, elem, direction).orThunk(() => { return Optional.some(elem); });
            });
        }
        return Optional.none();
    };
    ...
    const freefallRtl$1 = (universe, element) => {
        const candidate = scan$1(universe, element, universe.query().prevSibling).getOr(element);
        if (universe.property().isText(candidate)) {
            return point(candidate, toEnd(universe, candidate));
        }
        const children = universe.property().children(candidate);
        return children.length > 0 ? freefallRtl$1(universe, children[children.length - 1]) : point(candidate, toEnd(universe, candidate));
    };
    const freefallRtl = element => {
        return freefallRtl$1(DomUniverse(), element);
    };
    ...
    const resizeTable = () => {
        ...
        const calcRightEdgeDeltas = (sizes, _prev, index, delta, minCellSize, isRelative) => {
            if (isRelative) return calcRelativeDeltas(sizes, index, delta, minCellSize);
            return zero(sizes.slice(0, index)).concat([clampNegativeDelta(sizes, index, delta, minCellSize)]);
        };
        const calcRedestributedWidths = (sizes, totalWidth, pixelDelta, isRelative) => {
            if (isRelative) {
                ...
                return {delta: ratio * 100 - 100, newSizes};
            }
            return {delta: pixelDelta, newSizes: sizes};
        };
        return {resizeTable, clampTableDelta: clampNegativeDelta, calcLeftEdgeDeltas, calcMiddleDeltas, calcRightEdgeDeltas, calcRedestributedWidths};
    };
    const preserveTable = () => {
        ...
        const calcRightEdgeDeltas = (sizes, _prev, _index, delta, _minCellSize, isRelative) => {
            ...
            return map$1(sizes, delta / sizes.length);
        };
        const clampTableDelta = (sizes, index, delta, minCellSize, isLastColumn) => {
            if (isLastColumn) {
                if (delta >= 0) return delta;
                return Math.max(-foldl(sizes, (a, b) => a + b - minCellSize, 0), delta);
            }
            return clampNegativeDelta(sizes, index, delta, minCellSize);
        };
        const calcRedestributedWidths = (sizes, _totalWidth, _pixelDelta, _isRelative) => ({delta: 0, newSizes: sizes});
        return {
            resizeTable,
            clampTableDelta,
            calcLeftEdgeDeltas,
    ...
    const getGridSize = table => { return Warehouse.fromTable(table).grid; };
    ...
    const getRowType = row => {
        const isHeaderRow = row.section === 'thead', isHeaderCells = is(findCommonCellType(row.cells), 'th');
        if (row.section === 'tfoot') return {type: 'footer'};
        else if (isHeaderRow || isHeaderCells) return {type: 'header', subType: getRowHeaderType(isHeaderRow, isHeaderCells)};
        return {type: 'body'};
    };
    ...
    const sectionCells = () => ({transformRow, transformCell});
    const cells = () => ({
        transformRow: (row, section) => { return transformRow(row, section === 'thead' ? 'tbody' : section); },
        transformCell
    });
    const fallback = () => ({transformRow: identity, transformCell});
    ...
    const TableSection = {getTableSectionType, section, sectionCells, cells, fallback};
    ...
    const render$1 = (table, grid) => {
        ...
        const headSection = [], bodySection = [], footSection = [], columnGroupsSection = [];
        each$2(grid, row => {
            switch (row.section) {
                case 'thead':
    ...
    const getColumn = (grid, index) => { return map$1(grid, row => { return getCell(row, index); }); };
    const getRow = (grid, index) => { return grid[index]; };
    const findDiff = (xs, comp) => {
        if (xs.length === 0) return 0;
        const first = xs[0], index = findIndex(xs, x => { return !comp(first.element, x.element); });
        return index.getOr(xs.length);
    };
    const subgrid = (grid, row, column, comparator) => {
        const gridRow = getRow(grid, row), isColRow = gridRow.section === 'colgroup';
    ...
    const toGrid = (warehouse, generators, isNew) => {
        ...
        for (let rowIndex = 0; rowIndex < warehouse.grid.rows; rowIndex++) {
            ...
            grid.push(rowcells(rowDetail.element, rowCells, rowDetail.section, isNew));
        }
        return grid;
    };
    ...
    const extractCells = (warehouse, target, predicate) => {
        const details = map$1(target.selection, cell$1 => { return cell(cell$1).bind(lc => findInWarehouse(warehouse, lc)).filter(predicate); });
    ...
    const run = (operation, extract, adjustment, postAction, genWrappers) => (table, target, generators, behaviours) => {
        ...
        const output = extract(warehouse, target).map(info => {
            ...
            return {info, grid, cursor: result.cursor, lockedColumns};
        });
        return output.bind(out => {
            ...
            return Optional.some({cursor: out.cursor, newRows: newElements.newRows, newCells: newElements.newCells});
        });
    };
    const onPaste = (warehouse, target) => cell(target.element).bind(cell => findInWarehouse(warehouse, cell).map(details => {
        return {...details, generators: target.generators, clipboard: target.clipboard};
    }));
    const onPasteByEditor = (warehouse, target) => extractCells(warehouse, target, always).map(cells => ({cells, generators: target.generators, clipboard: target.clipboard}));
    ...
    const merge$2 = (grid, bounds, comparator, substitution) => {
        ...
        for (let i = bounds.startRow; i <= bounds.finishRow; i++) {
            for (let j = bounds.startCol; j <= bounds.finishCol; j++) {
                mutateCell(rows[i], j, elementnew(substitution(), false, getCell(rows[i], j).isLocked));
            }
        }
        return grid;
    };
    const unmerge = (grid, target, comparator, substitution) => {
        ...
        for (let i = 0; i < rows.length; i++) {
            for (let j = 0; j < cellLength(rows[0]); j++) {
                const currentCell = getCell(rows[i], j);
                const isToReplace = comparator(currentCell.element, target);
                if (isToReplace && !first) mutateCell(rows[i], j, elementnew(substitution(), true, currentCell.isLocked));
    ...
    const uniqueCells = (row, comparator) => {
        return foldl(row, (rest, cell) => {
            return exists(rest, currentCell => { return comparator(currentCell.element, cell.element); }) ? rest : rest.concat([cell]);
        }, []);
    };
    ...
    const splitRows = (grid, index, comparator, substitution) => {
        ...
        if (index > 0 && index < rows.length) {
            const rowPrevCells = rows[index - 1].cells, cells = uniqueCells(rowPrevCells, comparator);
            each$2(cells, cell => {
                let replacement = Optional.none();
                for (let i = index; i < rows.length; i++) {
                    for (let j = 0; j < cellLength(rows[0]); j++) {
                        const row = rows[i], current = getCell(row, j);
                        const isToReplace = comparator(current.element, cell.element);
                        if (isToReplace) {
                            if (replacement.isNone()) replacement = Optional.some(substitution());
                            replacement.each(sub => { mutateCell(row, j, elementnew(sub, true, current.isLocked)); });
                        }
                    }
                }
            });
    ...
    const value$1 = value => {
        const applyHelper = fn => fn(value), constHelper = constant(value), outputHelper = () => output;
        const output = {
            tag: true,
            inner: value,
    ...
    const Result = {value: value$1, error, fromOption};
    const measure = (startAddress, gridA, gridB) => {
        ...
        const rowRemainder = gridA.slice(startAddress.row), colRemainder = rowRemainder[0].cells.slice(startAddress.column);
        const colRequired = cellLength(gridB[0]), rowRequired = gridB.length;
        return Result.value({rowDelta: rowRemainder.length - rowRequired, colDelta: colRemainder.length - colRequired});
    };
    const measureWidth = (gridA, gridB) => {
        const colLengthA = cellLength(gridA[0]), colLengthB = cellLength(gridB[0]);
        return {rowDelta: 0, colDelta: colLengthA - colLengthB};
    };
    const measureHeight = (gridA, gridB) => { return {rowDelta: gridA.length - gridB.length, colDelta: 0}; };
    ...
    const colFill = (grid, amount, generators, startIndex) => map$1(grid, row => { return addCells(row, startIndex, generateElements(amount, row, generators, never)); });
    const lockedColFill = (grid, generators, lockedColumns) => map$1(grid, row => {
        return foldl(lockedColumns, (acc, colNum) => { return addCell(acc, colNum, generateElements(1, row, generators, always)[0]); }, row);
    });
    ...
    const isSpanning = (grid, row, col, comparator) => {
        const candidate = getCell(grid[row], col), matching = curry(comparator, candidate.element), currentRow = grid[row];
        return grid.length > 1 && cellLength(currentRow) > 1 && (col > 0 && matching(getCellElement(currentRow, col - 1)) || col < currentRow.cells.length - 1 && matching(getCellElement(currentRow, col + 1)) || row > 0 && matching(getCellElement(grid[row - 1], col)) || row < grid.length - 1 && matching(getCellElement(grid[row + 1], col)));
    };
    const mergeTables = (startAddress, gridA, gridBRows, generator, comparator, lockedColumns) => {
        const startRow = startAddress.row, startCol = startAddress.column, mergeHeight = gridBRows.length;
        const mergeWidth = cellLength(gridBRows[0]), endRow = startRow + mergeHeight;
        ...
        for (let r = startRow; r < endRow; r++) {
            ...
            for (let c = startCol; c < endCol; c++) {
                ...
                const newCellElm = newCell.element, replacement = generator.replace(newCellElm);
    ...
    const getValidStartAddress = (currentStartAddress, grid, lockedColumns) => {
        ...
        return {row: adjustedRowAddress, column: validColAddress};
    };
    ...
    const merge$1 = (startAddress, gridA, gridB, generator, comparator) => {
        ...
        return result.map(diff => {
            const delta = {...diff, colDelta: diff.colDelta - lockedColumnsWithinBounds.length};
            const fittedGrid = tailor(gridA, delta, generator), newLockedColumns = getLockedColumnsFromGrid(fittedGrid);
    ...
    const insertCols = (index, gridA, gridB, generator, comparator) => {
        ...
        return map$1(fittedOldGrid, (gridRow, i) => { return addCells(gridRow, index, fittedNewGrid[i].cells); });
    };
    const insertRows = (index, gridA, gridB, generator, comparator) => {
        ...
        const delta = {...diff, colDelta: diff.colDelta - locked.length};
        ...
        const secondDelta = {...secondDiff, colDelta: secondDiff.colDelta + newLocked.length};
    ...
    const insertRowAt = (grid, index, example, comparator, substitution) => {
        ...
        const before = rows.slice(0, index), after = rows.slice(index);
        const newRow = cloneRow(rows[example], (ex, c) => {
            ...
            return withinSpan ? getCell(rows[index], c) : elementnew(substitution(ex.element, comparator), true, ex.isLocked);
        }, comparator, substitution);
        return [...cols, ...before, newRow, ...after];
    };
    const getElementFor = (row, column, section, withinSpan, example, comparator, substitution) => {
        if (section === 'colgroup' || !withinSpan) return elementnew(substitution(getCell(row, example).element, comparator), true, false);
    ...
    const replaceIn = (grid, targets, comparator, substitute, replacer, genScope, shouldReplace) => {
        const isTarget = cell => { return exists(targets, target => { return comparator(cell.element, target.element); }); };
        return map$1(grid, (row, rowIndex) => {
    ...
    const replaceRows = (grid, indexes, section, applyScope, comparator, substitution, tableSection) => {
        ...
        const newRows = [...rows];
        each$2(indexes, index => { newRows[index] = tableSection.transformRow(rows[index], section); });
        ...
        each$2(cases, (acase, count) => {
            ...
            adt[key] = (...args) => {
                ...
                const match = branches => {
                    ...
                    const allReqd = forall(constructors, reqKey => { return contains$2(branchKeys, reqKey); });
                ...
                return {
                    ...
                    match,
                    log: label => { console.log(label, {constructors, constructor: key, params: args}); }
                };
            };
        });
        return adt;
    ...
    const determine = (input, column, step, tableSize, resize) => {
        const result = input.slice(0), context = neighbours(input, column);
    ...
    const recalculateWidthForCells = (warehouse, widths) => {
        ...
        return map$1(all, cell => {
            ...
            return {element: cell.element, width, colspan: cell.colspan};
        });
    };
    const recalculateWidthForColumns = (warehouse, widths) => {
        const groups = Warehouse.justColumns(warehouse);
        return map$1(groups, (column, index) => ({element: column.element, width: widths[index], colspan: column.colspan}));
    };
    const recalculateHeightForCells = (warehouse, heights) => {
        const all = Warehouse.justCells(warehouse);
        return map$1(all, cell => {
            const height = total(cell.row, cell.row + cell.rowspan, heights);
            return {element: cell.element, height, rowspan: cell.rowspan};
        });
    };
    const matchRowHeight = (warehouse, heights) => {
        return map$1(warehouse.all, (row, i) => {
            return {element: row.element, height: heights[i]};
        });
    };
    ...
    const recalculateAndApply = (warehouse, widths, tableSize) => {
        ...
        each$2(newSizes, cell => { tableSize.setElementWidth(cell.element, cell.width); });
    };
    ...
    const adjustHeight = (table, delta, index, direction) => {
        ...
        each$2(matchRowHeight(warehouse, newHeights), row => { setHeight(row.element, row.height); });
        each$2(recalculateHeightForCells(warehouse, newHeights), cell => { setHeight(cell.element, cell.height); });
    ...
    const uniqueColumns = details => {
        const uniqueCheck = (rest, detail) => { return exists(rest, currentDetail => currentDetail.column === detail.column) ? rest : rest.concat([detail]); };
        return foldl(details, uniqueCheck, []).sort((detailA, detailB) => detailA.column - detailB.column);
    };
    const isCol = isTag('col'), isColgroup = isTag('colgroup');
    const isRow$1 = element => name(element) === 'tr' || isColgroup(element);
    const elementToData = element => {
        const colspan = getAttrValue(element, 'colspan', 1);
        const rowspan = getAttrValue(element, 'rowspan', 1);
        return {element, colspan, rowspan};
    };
    const modification = (generators, toData = elementToData) => {
        ...
        const getOrInit = (element, comparator) => {
            return recent.fold(() => { return add(element); }, p => { return comparator(element, p.item) ? p.replacement : add(element); });
        };
        return {getOrInit};
    };
    const transform$1 = tag => {
        return generators => {
            ...
            const find = (element, comparator) => {
                return find$1(list, x => { return comparator(x.item, element); });
            };
            const makeNew = element => {
                const cell = generators.replace(element, tag, tag === 'td' ? {scope: null} : {});
                list.push({item: element, sub: cell});
                return cell;
            };
            const replaceOrInit = (element, comparator) => {
                if (isRow$1(element) || isCol(element)) return element;
                return find(element, comparator).fold(() => { return makeNew(element); }, p => { return comparator(element, p.item) ? p.sub : makeNew(element); });
            };
    ...
    const merging = generators => {
        const unmerge = cell => {
            getScopeAttribute(cell).each(attribute => set$2(cell, 'scope', attribute));
            return () => {
                const raw = generators.cell({element: cell, colspan: 1, rowspan: 1});
                remove$5(raw, 'width');
                remove$5(cell, 'width');
                getScopeAttribute(cell).each(attribute => set$2(raw, 'scope', attribute));
                return raw;
            };
        };
        const merge = cells => {
        ...
        return {unmerge, merge};
    };
    const Generators = {modification, transform: transform$1, merging};
    const blockList = [
        'body', 'p', 'div', 'article', 'aside', 'figcaption', 'figure', 'footer', 'header', 'nav', 'section', 'ol', 'ul',
        'table', 'thead', 'tfoot', 'tbody', 'caption', 'tr', 'td', 'th', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
        'blockquote', 'pre', 'address'
    ];
    const isList$1 = (universe, item) => { return contains$2(['ol', 'ul'], universe.property().name(item)); };
    const isBlock$1 = (universe, item) => { return contains$2(blockList, universe.property().name(item)); };
    ...
    const isBlock = element => { return isBlock$1(universe$1, element); };
    const isList = element => { return isList$1(universe$1, element); };
    const isEmptyTag = element => { return isEmptyTag$1(universe$1, element); };
    const merge = cells => {
        const isBr = isTag('br');
        const advancedBr = children => {
            return forall(children, c => { return isBr(c) || isText(c) && get$6(c).trim().length === 0; });
        };
        const isListItem = el => { return name(el) === 'li' || ancestor$2(el, isList).isSome(); };
        const siblingIsBlock = el => {
            return nextSibling(el).map(rightSibling => {
                if (isBlock(rightSibling)) return true;
                if (isEmptyTag(rightSibling)) return name(rightSibling) !== 'img';
                return false;
            }).getOr(false);
        };
        const markCell = cell => {
    ...
    const prune = table => {
        if (cells$1(table).length === 0) remove$6(table);
    };
    const outcome = (grid, cursor) => ({grid, cursor});
    const findEditableCursorPosition = rows => findMap(rows, row => findMap(row.cells, cell => {
        return someIf(isEditable(cell.element), cell.element);
    }));
    ...
    const bundle = (grid, row, column) => { return outcome(grid, elementFromGrid(grid, row, column)); };
    ...
    const opInsertRowsAfter = (grid, details, comparator, genWrappers) => {
        const rows = uniqueRows(details), target = rows[rows.length - 1], targetIndex = target.row + target.rowspan;
        const newGrid = foldr(rows, (newG, row) => { return insertRowAt(newG, targetIndex, row.row, comparator, genWrappers.getOrInit); }, grid);
        return bundle(newGrid, targetIndex, details[0].column);
    };
    const opInsertColumnsBefore = (grid, extractDetail, comparator, genWrappers) => {
        const details = extractDetail.details, columns = uniqueColumns(details), targetIndex = columns[0].column;
    ...
    const opInsertColumnsAfter = (grid, extractDetail, comparator, genWrappers) => {
        const details = extractDetail.details, target = details[details.length - 1];
        const targetIndex = target.column + target.colspan, columns = uniqueColumns(details);
    ...
    const opMakeColumnsHeader = (initialGrid, details, comparator, genWrappers) => {
        const columnIndexes = map$1(uniqueColumns(details), detail => detail.column);
    ...
    const makeRowsSection = (section, applyScope) => (initialGrid, details, comparator, genWrappers, tableSection) => {
        const rowIndexes = map$1(uniqueRows(details), detail => detail.row);
    ...
    const gridifyRows = (rows, generators, context) => {
        return toGrid(Warehouse.generate(fromPastedRows(rows, context.section)), generators, true);
    };
    const opPasteColsBefore = (grid, pasteDetails, comparator, _genWrappers) => {
        const context = extractGridDetails(grid).rows[pasteDetails.cells[0].row];
        const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
        const mergedGrid = insertCols(pasteDetails.cells[0].column, grid, gridB, pasteDetails.generators, comparator);
    ...
    const opPasteRowsBefore = (grid, pasteDetails, comparator, _genWrappers) => {
        ...
        const context = extractGridDetails(grid).rows[index];
        const gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
    ...
    const opGetCellsType = (table, target) => {
        return onCells(Warehouse.fromTable(table), target).bind(findCommonCellType).getOr('');
    };
    ...
    const getColumnsWidth = (warehouse, details) => {
        return foldl(uniqueColumns(details), (acc, detail) => { return acc + columns(warehouse)[detail.column].map(getOuter$2).getOr(0); }, 0);
    };
    ...
    }).map(details => ({details, pixelDelta: getColumnsWidth(warehouse, details)}));
    const eraseColumnsExtractor = (warehouse, target) => onUnlockedCells(warehouse, target).map(details => ({details, pixelDelta: -getColumnsWidth(warehouse, details)}));
    const pasteColumnsExtractor = before => (warehouse, target) => onPasteByEditor(warehouse, target).filter(details => {
        const checkLocked = before ? firstColumnIsLocked : lastColumnIsLocked;
        return !checkLocked(warehouse, details.cells);
    });
    const headerCellGenerator = Generators.transform('th'), bodyCellGenerator = Generators.transform('td');
    ...
    const getColumnsType = opGetColumnsType, getCellsType = opGetCellsType, getRowsType = opGetRowsType;
    ...
    const fireTableModified = (editor, table, data) => { editor.dispatch('TableModified', {...data, table}); };
    const fireTableSelectionChange = (editor, cells, start, finish, otherCells) => { editor.dispatch('TableSelectionChange', {cells, start, finish, otherCells}); };
    const fireTableSelectionClear = editor => { editor.dispatch('TableSelectionClear'); };
    const fireObjectResizeStart = (editor, target, width, height, origin) => { editor.dispatch('ObjectResizeStart', {target, width, height, origin}); };
    const fireObjectResized = (editor, target, width, height, origin) => { editor.dispatch('ObjectResized', {target, width, height, origin}); };
    ...
    const determineDefaultTableStyles = (editor, defaultStyles) => {
        ...
        else if (isTablePixelsForced(editor)) return {...defaultStyles, width: getPixelForcedWidth(editor)};
    ...
    const determineDefaultTableAttributes = (editor, defaultAttributes) => {
        ...
        else if (isTablePixelsForced(editor)) return {...defaultAttributes, width: getPixelForcedWidth(editor)};
        return {...defaultAttributes, width: defaultWidth};
    };
    const register = editor => {
        const registerOption = editor.options.register;
        registerOption('table_clone_elements', {processor: 'string[]'});
        registerOption('table_use_colgroups', {processor: 'boolean', default: true});
        ...
        registerOption('table_sizing_mode', {processor: 'string', default: 'auto'});
        registerOption('table_default_attributes', {processor: 'object', default: {border: '1'}});
        registerOption('table_default_styles', {processor: 'object', default: {'border-collapse': 'collapse'}});
        ...
        registerOption('table_resize_bars', {processor: 'boolean', default: true});
        registerOption('table_style_by_css', {processor: 'boolean', default: true});
    };
    const getTableCloneElements = editor => {
        return Optional.from(editor.options.get('table_clone_elements'));
    };
    const hasTableObjectResizing = editor => { return contains$2(editor.options.get('object_resizing').split(','), 'table'); };
    ...
    const getTableDefaultAttributes = editor => {
        const defaultAttributes = editor.options.get('table_default_attributes');
        return editor.options.isSet('table_default_attributes') ? defaultAttributes : determineDefaultTableAttributes(editor, defaultAttributes);
    };
    const getTableDefaultStyles = editor => {
        const defaultStyles = editor.options.get('table_default_styles');
        return editor.options.isSet('table_default_styles') ? defaultStyles : determineDefaultTableStyles(editor, defaultStyles);
    };
    ...
    const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
        ...
        const setSelectionFromAction = (table, result) => result.cursor.fold(() => {
            ...
            const des = freefallRtl(cell), rng = editor.dom.createRng();
        ...
        const execute = (operation, guard, mutate, effect) => (table, target, noEvents = false) => {
            ...
            return guard(table) ? operation(table, target, generators, behaviours).bind(result => {
                ...
                each$2(result.newRows, row => { fireNewRow(editor, row.dom); });
                each$2(result.newCells, cell => { fireNewCell(editor, cell.dom); });
                const range = setSelectionFromAction(table, result);
                if (inBody(table)) {
                    removeDataStyle(table);
                    if (!noEvents) fireTableModified(editor, table.dom, effect);
                }
                return range.map(rng => ({rng, effect}));
            }) : Optional.none();
        };
        ...
        const getTableCellType = getCellsType, getTableColType = getColumnsType, getTableRowType = getRowsType;
        return {
            deleteRow,
            deleteColumn,
    ...
    const isColInRange = (minColRange, maxColRange) => cell => {
        return cell.column + cell.colspan - 1 >= minColRange && cell.column < maxColRange;
    };
    ...
    const copyCols = (table, target) => {
        ...
        return details.map(selectedCells => {
            const lastSelectedCell = selectedCells[selectedCells.length - 1];
            const minColRange = selectedCells[0].column, maxColRange = lastSelectedCell.column + lastSelectedCell.colspan;
    ...
    const Size = {...adt$5, from};
    const redistributeToPercent = (widths, totalWidth) => {
        return map$1(widths, w => {
            const colType = Size.from(w);
            return colType.fold(() => { return w; }, px => { return (px / totalWidth * 100) + '%'; }, pc => { return pc + '%'; });
        });
    };
    const redistributeToPx = (widths, totalWidth, newTotalWidth) => {
        return map$1(widths, w => {
            return Size.from(w).fold(() => { return w; }, px => { return px * (newTotalWidth / totalWidth) + 'px'; }, pc => { return pc / 100 * newTotalWidth + 'px'; });
        });
    };
    const redistributeEmpty = (newWidthType, columns) => {
        const f = newWidthType.fold(() => constant(''), pixels => {
            return constant((pixels / columns) + 'px');
        }, () => {
            return constant((100 / columns) + '%');
        });
        return range$1(columns, f);
    };
    const redistributeValues = (newWidthType, widths, totalWidth) => {
        return newWidthType.fold(() => { return widths; }, px => { return redistributeToPx(widths, totalWidth, px); }, _pc => { return redistributeToPercent(widths, totalWidth); });
    };
    ...
    const roundDown = (num, unit) => {
        return {value: Math.floor(num) + unit, remainder: num - Math.floor(num)};
    };
    const add$3 = (value, amount) => {
        return Size.from(value).fold(constant(value), px => { return px + amount + 'px'; }, pc => { return pc + amount + '%'; });
    };
    const normalize = values => {
        ...
        const scan = foldr(values, (rest, value) => {
            const info = Size.from(value).fold(() => ({
                value,
                remainder: 0
            }), num => roundDown(num, 'px'), num => ({value: num + '%', remainder: 0}));
    ...
    const redistributeToW = (newWidths, cells, unit) => {
        each$2(cells, cell => {
            ...
            set$1(cell.element, 'width', sum(widths, minWidth()) + unit);
        });
    };
    const redistributeToColumns = (newWidths, columns, unit) => {
        each$2(columns, (column, index) => {
            set$1(column.element, 'width', sum([newWidths[index]], minWidth()) + unit);
        });
    };
    ...
    const redistribute = (table, optWidth, optHeight) => {
        const warehouse = Warehouse.fromTable(table), rows = warehouse.all;
        const cells = Warehouse.justCells(warehouse), columns = Warehouse.justColumns(warehouse);
        optWidth.each(newWidth => {
            const widthUnit = getUnit(newWidth), totalWidth = get$9(table);
    ...
    const isPercentSizing = isPercentSizing$1, isPixelSizing = isPixelSizing$1, isNoneSizing = isNoneSizing$1;
    const cleanupLegacyAttributes = element => { remove$7(element, 'width'); };
    ...
    const convertToNoneSize = table => {
        ...
        each$2(columns$1(table).length > 0 ? columns$1(table) : cells$1(table), cell => {
            remove$5(cell, 'width');
    ...
    const render = (rows, columns, rowHeaders, columnHeaders, headerType, renderOpts = DefaultRenderOptions) => {
        const table = SugarElement.fromTag('table'), rowHeadersGoInThead = headerType !== 'cells';
    ...
    const insertTable = (editor, rows, columns, options = {}) => {
        ...
        if (checkInput(rows) && checkInput(columns)) return insert(editor, columns, rows, options.headerColumns || 0, options.headerRows || 0);
        console.error('Invalid values for mceInsertTable - rows and columns values are required to insert a table.');
        return null;
    };
    let global = tinymce.util.Tools.resolve('tinymce.FakeClipboard');
    const tableTypeBase = 'x-tinymce/dom-table-', tableTypeRow = tableTypeBase + 'rows';
    const tableTypeColumn = tableTypeBase + 'columns';
    const setData = items => { global.write([global.FakeClipboardItem(items)]); };
    const getData = type => {
        let _a;
    ...
    const setRows = rowsOpt => { rowsOpt.fold(clearRows, rows => setData({[tableTypeRow]: rows})); };
    const getRows = () => getData(tableTypeRow);
    const clearRows = () => clearData(tableTypeRow);
    const setColumns = columnsOpt => { columnsOpt.fold(clearColumns, columns => setData({[tableTypeColumn]: columns})); };
    ...
    const registerCommands = (editor, actions) => {
        ...
        const postExecute = _data => { editor.focus(); };
        ...
        const copyRowSelection = () => performActionOnSelection((table, startCell) => {
            ...
            return copyRows(table, forMenu(getCellsFromSelection(editor), table, startCell), generators);
        });
        const copyColSelection = () => performActionOnSelection((table, startCell) => {
            return copyCols(table, forMenu(getCellsFromSelection(editor), table, startCell));
        });
    ...
    const adt$4 = Adt.generate([{before: ['element']}, {on: ['element', 'offset']}, {after: ['element']}]);
    const cata$1 = (subject, onBefore, onOn, onAfter) => subject.fold(onBefore, onOn, onAfter);
    const getStart$1 = situ => situ.fold(identity, identity, identity);
    const before$2 = adt$4.before, on = adt$4.on, after$3 = adt$4.after;
    const Situ = {before: before$2, on, after: after$3, cata: cata$1, getStart: getStart$1};
    const create$4 = (selection, kill) => ({selection, kill});
    ...
    const getRanges = (win, selection) => selection.match({
        domRange: rng => {return {ltr: constant(rng), rtl: Optional.none};},
    ...
    const diagnose = (win, selection) => { return doDiagnose(win, getRanges(win, selection)); };
    ...
    const create$2 = (start, soffset, finish, foffset) => {
        return {start: Situ.on(start, soffset), finish: Situ.on(finish, foffset)};
    };
    ...
    const traverse = (item, mode) => ({item, mode});
    const backtrack = (universe, item, _direction, transition = sidestep) => {
        return universe.property().parent(item).map(p => { return traverse(p, transition); });
    };
    const sidestep = (universe, item, direction, transition = advance) => {
        return direction.sibling(universe, item).map(p => { return traverse(p, transition); });
    };
    const advance = (universe, item, direction, transition = advance) => {
        const children = universe.property().children(item);
        return direction.first(children).map(r => { return traverse(r, transition); });
    };
    const successors = [
    ...
    const go = (universe, item, mode, direction, rules = successors) => {
        const ruleOpt = find$1(rules, succ => { return succ.current === mode; });
    ...
    const left$1 = () => {
        const sibling = (universe, item) => { return universe.query().prevSibling(item); };
        const first = children => { return children.length > 0 ? Optional.some(children[children.length - 1]) : Optional.none(); };
        return {sibling, first};
    };
    const right$1 = () => {
        const sibling = (universe, item) => { return universe.query().nextSibling(item); };
        const first = children => { return children.length > 0 ? Optional.some(children[0]) : Optional.none(); };
        return {sibling, first};
    };
    const Walkers = {left: left$1, right: right$1};
    ...
    const left = (universe, item, predicate, isRoot) => { return hone(universe, item, predicate, sidestep, Walkers.left(), isRoot); };
    const right = (universe, item, predicate, isRoot) => { return hone(universe, item, predicate, sidestep, Walkers.right(), isRoot); };
    const isLeaf = universe => element => universe.property().children(element).length === 0;
    const before$1 = (universe, item, isRoot) => { return seekLeft$1(universe, item, isLeaf(universe), isRoot); };
    const after$2 = (universe, item, isRoot) => { return seekRight$1(universe, item, isLeaf(universe), isRoot); };
    const seekLeft$1 = left, seekRight$1 = right, universe = DomUniverse();
    const before = (element, isRoot) => { return before$1(universe, element, isRoot); };
    const after$1 = (element, isRoot) => { return after$2(universe, element, isRoot); };
    const seekLeft = (element, predicate, isRoot) => { return seekLeft$1(universe, element, predicate, isRoot); };
    const seekRight = (element, predicate, isRoot) => { return seekRight$1(universe, element, predicate, isRoot); };
    ...
    const cata = (subject, onNone, onSuccess, onFailedUp, onFailedDown) => { return subject.fold(onNone, onSuccess, onFailedUp, onFailedDown); };
    const BeforeAfter = {...adt$2, verify, cata};
    const inParent = (parent, children, element, index) => ({parent, children, element, index});
    ...
    const handleParent = (isRoot, element, offset, direction) => {
        return findBr(element, offset).bind(br => {
            return direction.traverse(br).fold(() => {
                ...
                return indexInParent(adjacent).map(info => { return Situ.on(info.parent, info.index); });
            });
        });
    };
    const tryBr = (isRoot, element, offset, direction) => {
        ...
        return target.map(tgt => { return {start: tgt, finish: tgt}; });
    };
    ...
    const moveDown = (caret, amount) => { return {left: caret.left, top: caret.top + amount, right: caret.right, bottom: caret.bottom + amount}; };
    const moveUp = (caret, amount) => { return {left: caret.left, top: caret.top - amount, right: caret.right, bottom: caret.bottom - amount}; };
    const translate = (caret, xDelta, yDelta) => { return {left: caret.left + xDelta, top: caret.top + yDelta, right: caret.right + xDelta, bottom: caret.bottom + yDelta}; };
    const getTop = caret => { return caret.top; };
    const getBottom = caret => { return caret.bottom; };
    ...
    const toCaret = rect => ({left: rect.left, top: rect.top, right: rect.right, bottom: rect.bottom});
    const getElemBox = (bridge, element) => { return Optional.some(bridge.getRect(element)); };
    ...
    const JUMP_SIZE = 5, NUM_RETRIES = 100;
    const adt$1 = Adt.generate([{none: []}, {retry: ['caret']}]);
    const isOutside = (caret, box) => { return caret.left < box.left || Math.abs(box.right - caret.left) < 1 || caret.left > box.right; };
    const inOutsideBlock = (bridge, element, caret) => {
        return closest$2(element, isBlock).fold(never, cell => {
            return getEntireBox(bridge, cell).exists(box => { return isOutside(caret, box); });
        });
    };
    ...
    const upMovement = {point: getTop, adjuster: adjustUp, move: moveUp, gather: before};
    const downMovement = {point: getBottom, adjuster: adjustDown, move: moveDown, gather: after$1};
    const isAtTable = (bridge, x, y) => { return bridge.elementFromPoint(x, y).filter(elm => { return name(elm) === 'table'; }).isSome(); };
    const adjustForTable = (bridge, movement, original, caret, numRetries) => { return adjustTil(bridge, movement, original, movement.move(caret, JUMP_SIZE), numRetries); };
    const adjustTil = (bridge, movement, original, caret, numRetries) => {
        ...
        return bridge.situsFromPoint(caret.left, movement.point(caret)).bind(guess => {
            return guess.start.fold(Optional.none, element => {
                ...
                }).orThunk(() => { return Optional.some(caret); });
            }, Optional.none);
        });
    };
    ...
    const Retries = {tryUp: curry(retry, upMovement), tryDown: curry(retry, downMovement), getJumpSize: constant(JUMP_SIZE)};
    ...
    const inSameTable = (elem, table) => {
        return ancestor(elem, e => {
            return parent(e).exists(p => { return eq$1(p, table); });
        });
    };
        ...
        return closest$1(initial, 'td,th', isRoot).bind(start => {
            return closest$1(start, 'table', isRoot).bind(table => {
                ...
                return handle$1(bridge, isRoot, direction).bind(range => {
                    return closest$1(range.finish, 'td,th', isRoot).map(finish => { return {start, finish, range}; });
                });
            });
        });
    };
    ...
    const MouseSelection = (bridge, container, isRoot, annotations) => {
        const cursor = value(), clearstate = cursor.clear;
        ...
        return {clearstate, mousedown, mouseover, mouseup};
    };
    const down = {traverse: nextSibling, gather: after$1, relative: Situ.before, retry: Retries.tryDown, failure: BeforeAfter.failedDown};
    const up = {traverse: prevSibling, gather: before, relative: Situ.before, retry: Retries.tryUp, failure: BeforeAfter.failedUp};
    const isKey = key => { return keycode => { return keycode === key; }; };
    const isUp = isKey(38), isDown = isKey(40);
    const isNavigation = keycode => { return keycode >= 37 && keycode <= 40; };
    const ltr = {isBackward: isKey(37), isForward: isKey(39)};
    const rtl = {isBackward: isKey(39), isForward: isKey(37)};
    const get$3 = _DOC => {
        const doc = _DOC !== undefined ? _DOC.dom : document, x = doc.body.scrollLeft || doc.documentElement.scrollLeft;
        const y = doc.body.scrollTop || doc.documentElement.scrollTop;
        return SugarPosition(x, y);
    };
    const by = (x, y, _DOC) => {
        const doc = _DOC !== undefined ? _DOC.dom : document;
        if (doc.defaultView) doc.defaultView.scrollBy(x, y);
    };
    ...
    const domRange = adt.domRange, relative = adt.relative, exact = adt.exact;
    const getWin = selection => { return defaultView(getStart(selection)); };
    const range = SimRange.create;
    const SimSelection = {domRange, relative, exact, exactFromRange, getWin, range};
    ...
    const preprocessExact = (start, soffset, finish, foffset) => {
        const startSitu = beforeSpecial(start, soffset), finishSitu = beforeSpecial(finish, foffset);
    ...
    const doSetRange = (win, start, soffset, finish, foffset) => { doSetNativeRange(win, exactToNative(win, start, soffset, finish, foffset)); };
    ...
    const setRangeFromRelative = (win, relative) => diagnose(win, relative).match({
        ltr: (start, soffset, finish, foffset) => { doSetRange(win, start, soffset, finish, foffset); },
        rtl: (start, soffset, finish, foffset) => {
            getNativeSelection(win).each(selection => {
    ...
    const setExact = (win, start, soffset, finish, foffset) => { setRangeFromRelative(win, preprocessExact(start, soffset, finish, foffset)); };
    const setRelative = (win, startSitu, finishSitu) => { setRangeFromRelative(win, preprocessRelative(startSitu, finishSitu)); };
    ...
    const setToElement = (win, element, selectNodeContents$1 = true) => {
        ...
        doSetNativeRange(win, rngGetter(win, element));
    };
    ...
    const getFirstRect = (win, selection) => { return getFirstRect$1(asLtrRange(win, selection)); };
    const getAtPoint = (win, x, y) => fromPoint(win, x, y);
    const clear = win => { getNativeSelection(win).each(selection => selection.removeAllRanges()); };
    const WindowBridge = win => {
        const elementFromPoint = (x, y) => { return SugarElement.fromPoint(SugarElement.fromDom(win.document), x, y); };
        const getRect = element => { return element.dom.getBoundingClientRect(); };
        const getRangedRect = (start, soffset, finish, foffset) => { return getFirstRect(win, SimSelection.exact(start, soffset, finish, foffset)); };
        const getSelection = () => { return get$2(win).map(exactAdt => { return convertToRange(win, exactAdt); }); };
        const fromSitus = situs => { return convertToRange(win, SimSelection.relative(situs.start, situs.finish)); };
        const situsFromPoint = (x, y) => {
            return getAtPoint(win, x, y).map(exact => {
                return Situs.create(exact.start, exact.soffset, exact.finish, exact.foffset);
            });
        };
        const clearSelection = () => { clear(win); };
        const collapseSelection = (toStart = false) => {
            get$2(win).each(sel => sel.fold(rng => rng.collapse(toStart), (startSitu, finishSitu) => {
                const situ = toStart ? startSitu : finishSitu;
                setRelative(win, situ, situ);
            }, (start, soffset, finish, foffset) => {
                const node = toStart ? start : finish, offset = toStart ? soffset : foffset;
                setExact(win, node, offset, node, offset);
            }));
        };
        const selectNode = element => { setToElement(win, element, false); };
        const selectContents = element => { setToElement(win, element); };
        const setSelection = sel => { setExact(win, sel.start, sel.soffset, sel.finish, sel.foffset); };
        const setRelativeSelection = (start, finish) => { setRelative(win, start, finish); };
        const getInnerHeight = () => { return win.innerHeight; };
        const getScrollY = () => { return get$3(SugarElement.fromDom(win.document)).top; };
        const scrollBy = (x, y) => { by(x, y, SugarElement.fromDom(win.document)); };
        return {
            elementFromPoint,
            getRect,
            getRangedRect,
    ...
    const mouse = (win, container, isRoot, annotations) => {
        const bridge = WindowBridge(win), handlers = MouseSelection(bridge, container, isRoot, annotations);
        return {
    ...
    const keyboard = (win, container, isRoot, annotations) => {
        ...
        const keyup = (event, start, soffset, finish, foffset) => {
            return retrieve$1(container, annotations.selectedSelector).fold(() => {
                const realEvent = event.raw, keycode = realEvent.which, shiftKey = realEvent.shiftKey === true;
    ...
    const remove$1 = (element, classes) => {
        each$2(classes, x => { remove$2(element, x); });
    };
    const addClass = clazz => element => { add(element, clazz); };
    const removeClasses = classes => element => { remove$1(element, classes); };
    const byClass = ephemera => {
        ...
        const clear = container => { each$2(descendants(container, ephemera.selectedSelector), removeSelectionClasses); };
    ...
    const byAttr = (ephemera, onSelection, onClear) => {
        ...
        const addSelectionAttribute = element => { set$2(element, ephemera.selected, '1'); };
        const clear = container => { clearBeforeUpdate(container); onClear(); };
    ...
    const SelectionAnnotation = {byClass, byAttr};
    ...
    const getUpOrLeftCells = (grid, selectedCells) => {
        const upGrid = grid.slice(0, selectedCells[selectedCells.length - 1].row + 1), upDetails = toDetailList(upGrid);
    ...
    const getOtherCells = (table, target, generators) => {
        ...
        return details.map(selectedCells => {
            ...
            return {upOrLeftCells, downOrRightCells};
        });
    };
    const mkEvent = (target, x, y, stop, prevent, kill, raw) => ({target, x, y, stop, prevent, kill, raw});
    ...
    const unbind = (element, event, handler, useCapture) => { element.dom.removeEventListener(event, handler, useCapture); };
    const filter = always;
    ...
    const TableCellSelectionHandler = (editor, resizeHandler) => {
        ...
        editor.on('init', _e => {
            const win = editor.getWin(), body = getBody(editor), isRoot = getIsRoot(editor);
            const syncSelection = () => {
                const sel = editor.selection, start = SugarElement.fromDom(sel.getStart());
            ...
            const handleResponse = (event, response) => {
                ...
                response.selection.each(ns => { editor.selection.setRng(asLtrRange(win, SimSelection.relative(ns.start, ns.finish))); });
            };
            const keyup = event => {
            ...
            const dragStart = _e => { mouseHandlers.clearstate(); };
            const mouseDown = e => {
            ...
            const getDoubleTap = () => {
                ...
                const touchEnd = t => {
                    ...
                    if (isTag('td')(target) || isTag('th')(target)) {
                        const lT = lastTarget.get(), lTS = lastTimeStamp.get();
        ...
        return {getSelectedCells, clearSelectedCells};
    };
    const Event = fields => {
        let handlers = [];
        const bind = handler => {
        ...
        const unbind = handler => {
            handlers = filter$2(handlers, h => { return h !== handler; });
        };
        const trigger = (...args) => {
            const event = {};
            each$2(fields, (name, i) => { event[name] = args[i]; });
            each$2(handlers, handler => { handler(event); });
        };
        return {bind, unbind, trigger};
    };
    const create$1 = typeDefs => {
        const registry = map(typeDefs, event => { return {bind: event.bind, unbind: event.unbind}; });
        const trigger = map(typeDefs, event => { return event.trigger; });
    ...
    const sort = arr => { return arr.slice(0).sort(); };
    const reqMessage = (required, keys) => { throw new Error('All required keys (' + sort(required).join(', ') + ') were not specified. Specified keys were: ' + sort(keys).join(', ') + '.'); };
    const unsuppMessage = unsupported => { throw new Error('Unsupported keys for object: ' + sort(unsupported).join(', ')); };
    ...
    const invalidTypeMessage = (incorrect, type) => { throw new Error('All values need to be of type: ' + type + '. Keys (' + sort(incorrect).join(', ') + ') were not.'); };
    const checkDupes = everything => {
        const sorted = sort(everything);
        const dupe = find$1(sorted, (s, i) => { return i < sorted.length - 1 && s === sorted[i + 1]; });
        dupe.each(d => { throw new Error('The field: ' + d + ' occurs more than once in the combined fields: [' + sorted.join(', ') + '].'); });
    };
    const base = (handleUnsupported, required) => {
        return baseWith(handleUnsupported, required, {validate: isFunction, label: 'function'});
    };
    const baseWith = (handleUnsupported, required, pred) => {
        ...
        checkDupes(required);
        return obj => {
            const keys$1 = keys(obj), allReqd = forall(required, req => { return contains$2(keys$1, req); });
            if (!allReqd) reqMessage(required, keys$1);
            handleUnsupported(required, keys$1);
            const invalidKeys = filter$2(required, key => { return !pred.validate(obj[key], key); });
            if (invalidKeys.length > 0) invalidTypeMessage(invalidKeys, pred.label);
            return obj;
        };
    };
    const handleExact = (required, keys) => {
        const unsupported = filter$2(keys, key => { return !contains$2(required, key); });
    ...
    const InDrag = () => {
        ...
        const reset = () => { previous = Optional.none(); };
        const update = (mode, nu) => {
            const result = previous.map(old => { return mode.compare(old, nu); });
        ...
        const onEvent = (event, mode) => {
            ...
            dataOption.each(data => {
                ...
                offset.each(d => { events.trigger.move(d); });
            });
        };
        const events = create$1({move: Event(['info'])});
        return {onEvent, reset, events: events.registry};
    };
    const NoDrag = () => {
        const events = create$1({move: Event(['info'])});
        return {onEvent: noop, reset: noop, events: events.registry};
    };
    const Movement = () => {
        const noDragState = NoDrag(), inDragState = InDrag();
        ...
        const onEvent = (event, mode) => { dragState.onEvent(event, mode); };
        const isOn = () => { return dragState === inDragState; };
        return {on, off, isOn, onEvent, events: inDragState.events};
    };
    const setup = (mutation, mode, settings) => {
        let active = false;
        const events = create$1({start: Event([]), stop: Event([])});
        ...
        const sink = mode.sink(DragApi({forceDrop: drop, drop: runIfActive(drop), move: runIfActive(mousemove), delayDrop: runIfActive(throttledDrop.throttle)}), settings);
        const destroy = () => { sink.destroy(); };
        return {element: sink.element, go, on, off, isActive, destroy, events: events.registry};
    };
    const css = namespace => {
        const dashNamespace = namespace.replace(/\./g, '-');
        const resolve = str => { return dashNamespace + '-' + str; };
        return {resolve};
    };
    const styles$1 = css('ephox-dragster'), resolve$1 = styles$1.resolve;
    const Blocker = options => {
        const settings = {layerClass: resolve$1('blocker'), ...options};
        const div = SugarElement.fromTag('div');
        set$2(div, 'role', 'presentation');
        setAll(div, {position: 'fixed', left: '0px', top: '0px', width: '100%', height: '100%'});
        add(div, resolve$1('blocker'));
        add(div, settings.layerClass);
        const element = constant(div), destroy = () => { remove$6(div); };
        return {element, destroy};
    };
    const compare = (old, nu) => { return SugarPosition(nu.left - old.left, nu.top - old.top); };
    const extract = event => { return Optional.some(SugarPosition(event.x, event.y)); };
    const mutate = (mutation, info) => { mutation.mutate(info.left, info.top); };
    const sink = (dragApi, settings) => {
        ...
        const start = parent => { append$1(parent, blocker.element()); };
        const stop = () => { remove$6(blocker.element()); };
        return DragSink({element: blocker.element, start, stop, destroy});
    };
    let MouseDrag = DragMode({compare, extract, sink, mutate});
    const transform = (mutation, settings = {}) => {
        let _a;
        const mode = (_a = settings.mode) !== null && _a !== void 0 ? _a : MouseDrag;
        return setup(mutation, mode, settings);
    };
    const styles = css('ephox-snooker'), resolve = styles.resolve;
    const Mutation = () => {
        const events = create$1({drag: Event(['xDelta', 'yDelta'])});
        const mutate = (x, y) => { events.trigger.drag(x, y); };
        return {mutate, events: events.registry};
    };
    const BarMutation = () => {
        const events = create$1({drag: Event(['xDelta', 'yDelta', 'target'])});
        let target = Optional.none();
        const delegate = Mutation();
        delegate.events.drag.bind(event => {
            target.each(t => { events.trigger.drag(event.xDelta, event.yDelta, t); });
        });
        const assign = t => { target = Optional.some(t); };
        const get = () => { return target; };
        return {assign, get, mutate: delegate.mutate, events: events.registry};
    };
    const col = (column, x, y, w, h) => {
        const bar = SugarElement.fromTag('div');
        setAll(bar, {position: 'absolute', left: x - w / 2 + 'px', top: y + 'px', height: h + 'px', width: w + 'px'});
        setAll$1(bar, {'data-column': column, 'role': 'presentation'});
        return bar;
    };
    const row = (r, x, y, w, h) => {
        const bar = SugarElement.fromTag('div');
        setAll(bar, {position: 'absolute', left: x + 'px', top: y - h / 2 + 'px', height: h + 'px', width: w + 'px'});
        setAll$1(bar, {'data-row': r, 'role': 'presentation'});
        return bar;
    };
    const resizeBar = resolve('resizer-bar'), resizeRowBar = resolve('resizer-rows');
    const resizeColBar = resolve('resizer-cols'), BAR_THICKNESS = 7;
    ...
    const destroy = wire => { each$2(descendants(wire.parent(), '.' + resizeBar), remove$6); };
    ...
    const refresh = (wire, table) => {
        ...
        if (wire.isResizable(table)) {
            const warehouse = Warehouse.fromTable(table);
            refreshGrid(warehouse, wire, table, rows(warehouse), columns(warehouse));
        }
    };
    const each = (wire, f) => { each$2(descendants(wire.parent(), '.' + resizeBar), f); };
    const hide = wire => {
        each(wire, bar => { set$1(bar, 'display', 'none'); });
    };
    const show = wire => {
        each(wire, bar => { set$1(bar, 'display', 'block'); });
    };
    const isRowBar = element => { return has(element, resizeRowBar); };
    const isColBar = element => { return has(element, resizeColBar); };
    ...
    const BarManager = wire => {
        ...
        const isRoot = e => { return eq$1(e, wire.view()); };
        ...
        const refresh$1 = tbl => { refresh(wire, tbl); };
    ...
    const create = (wire, resizing, lazySizing) => {
        const hdirection = height, vdirection = width, manager = BarManager(wire);
        const events = create$1({
        ...
        manager.events.startAdjust.bind(_event => { events.trigger.startDrag(); });
        manager.events.adjustWidth.bind(event => {
            const table = event.table;
            events.trigger.beforeResize(table, 'col');
            const delta = vdirection.delta(event.delta, table), tableSize = lazySizing(table);
    ...
    const only = (element, isResizable) => {
        ...
        return {parent: constant(parent), view: constant(element), origin: constant(SugarPosition(0, 0)), isResizable};
    };
    const detached = (editable, chrome, isResizable) => {
        const origin = () => absolute(chrome);
        return {parent: constant(chrome), view: constant(editable), origin, isResizable};
    };
    const body = (editable, chrome, isResizable) => {
        return {parent: constant(chrome), view: constant(editable), origin: constant(SugarPosition(0, 0)), isResizable};
    };
    const ResizeWire = {only, detached, body};
    const createContainer = () => {
        const container = SugarElement.fromTag('div');
        setAll(container, {position: 'static', height: '0', width: '0', padding: '0', margin: '0', border: '0'});
    ...
    const TableResizeHandler = editor => {
        const selectionRng = value(), tableResize = value(), resizeWire = value();
        ...
        const afterCornerResize = (table, origin, width) => {
            ...
            if (width !== startW && startRawW !== '') {
                ...
                const resizing = lazyResizingBehaviour(), tableSize = lazySizing(table);
                const col = isPreserveTableColumnResizing(editor) || isRightEdgeResize ? getNumColumns(table) - 1 : 0;
                adjustWidth(table, width - startW, col, resizing, tableSize);
            } else if (isPercentage$1(startRawW)) {
                const percentW = parseFloat(startRawW.replace('%', '')), targetPercentW = width * percentW / startW;
                set$1(table, 'width', targetPercentW + '%');
            }
            if (isPixel(startRawW)) syncPixels(table);
        };
        const destroy = () => {
            tableResize.on(sz => { sz.destroy(); });
            resizeWire.on(w => { remove(editor, w); });
        };
        editor.on('init', () => {
            const rawWire = get(editor, isResizable);
            resizeWire.set(rawWire);
            if (hasTableObjectResizing(editor) && hasTableResizeBars(editor)) {
                const resizing = lazyResizingBehaviour(), sz = TableResize.create(rawWire, resizing, lazySizing);
                sz.on();
                sz.events.startDrag.bind(_event => { selectionRng.set(editor.selection.getRng()); });
        ...
        editor.on('ObjectResized', e => {
            const targetElm = e.target;
            if (isTable(targetElm)) {
                const table = SugarElement.fromDom(targetElm), origin = e.origin;
        ...
        editor.on('remove', () => { destroy(); });
        const refresh = table => { tableResize.on(resize => resize.refreshBars(SugarElement.fromDom(table))); };
        const hide = () => { tableResize.on(resize => resize.hideBars()); };
        const show = () => { tableResize.on(resize => resize.showBars()); };
    ...
    const setupTable = editor => {
        ...
        return {getSelectedCells: cellSelectionHandler.getSelectedCells, clearSelectedCells: cellSelectionHandler.clearSelectedCells};
    };
    const DomModel = editor => {
        const table = setupTable(editor);
        return {table};
    };
    let Model = () => { global$1.add('dom', DomModel); };
    Model();
})();